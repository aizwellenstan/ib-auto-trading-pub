...
from modules.morningstar import GetExDividendASX, GetExDividendNZE
import datetime
...
FOLDER = f"{rootPath}/data/dividendHunter/asx"
...
def GetClose(symbol):
    ...
    close = GetDataWithVolumeDate(symbol, market='AX')[-1][1]
    return close
...
def GetStockCFDSpread(symbol):
    cStock = Stock(symbol, 'SMART', 'AUD')
    cCFD = CFD(symbol, 'SMART', 'AUD')
...
def UpdateDivPer(ex_filename, divPerDict):
    ...
    for symbol, close, divPer, div in npArr:
        ...
        # contract = CFD(symbol, 'SMART', 'AUD')
        # ib.qualifyContracts(contract)
        # c = ib.reqContractDetails(contract)
        # if len(c) < 1: continue
        close = GetClose(symbol)
...
# https://www.morningstar.com.au/markets/upcoming-dividends
def main():
    exDate = datetime.datetime.now().strftime('%Y-%m-%d')
    # print(exDate)
    # exDate = "2024-11-05"
    ...
    if not os.path.exists(div_per_filename):
        if not os.path.exists(ex_filename):
            ...
            dfAsx = GetExDividendASX()
            dfNze = GetExDividendNZE()
            df = pd.concat([dfAsx, dfNze], ignore_index=True, sort=False)
            
            df = df.loc[df['exDate'] == exDate]
            df = df[["ticker","divCashAmount"]]
        ...
        closeDict = {}
        divPerDict = {}
        for symbol, div in npArr:
            # contract = CFD(symbol, 'SMART', 'AUD')
            # ib.qualifyContracts(contract)
            # c = ib.reqContractDetails(contract)
            # hasCFD = False
            # # check has cfd contract if has higher priority
            # if len(c) > 0: hasCFD = True
            close = GetClose(symbol)
            print(symbol, close)
            ...
            closeDict[symbol] = close
            divPer = div / close
            divPerDict[symbol] = [close, div, divPer]
    ...
    print(divPerDict)
    print(avalible_cash)
    divPerDict = UpdateDivPer(div_per_filename, divPerDict)

    # temp
    for symbol, data in divPerDict.items():
        vol = int(avalible_cash / data[0])
        div = data[1]
        totalDivs = vol * div
        commission = 10
        profit = totalDivs - commission
        print(symbol, vol, totalDivs, profit)
    sys.exit()
    ...
    for symbol, data in divPerDict.items():
        ...
        commissionAndSpread = (11 + (spreadStock+spreadCFD) * sharesToBuy) * 2
