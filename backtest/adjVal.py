...
from modules.sharpe import GetSharpe, GetStd, GetCalmar, GetMaxDD
...
def main():
    ...
    try:
        ...
        for trade in trades:
            ...
            dateSym = str(backtestTime.date()) + symbol
                            ...
                            if eod:
                                if i.date >= endTime:
                                    if(i.open > op):
                                        ...
                                        total += net
                                    break
            dateSymProfitDict[dateSym] = total
        ...
        while adjVal <= 65.29:
            ...
            for trade in trades:
                ...
                if symDf.iloc[-1].Open < symDf.iloc[-2].Close * 1.02: 
                    continue
                if symbol in shortableSymList:
                    # if symbol in dateSymDfDict:
                    #     symDf = dateSymDfDict[dateSym]
                    # else:
                    #     symDf = vwapDfDict[symbol]
                    #     mask = symDf.Date < str(backtestTime.date())
                    #     symDf = symDf.loc[mask]
                    ...
                    # if lower < adjVal: continue
                    
                    # df = symDf
                    # df = df[['Close']]
                    # df = df.tail(27)
                    # sortino = GetMaxDD(df)
                    # if sortino < adjVal: continue
                    ...
                    # gapRange = symDf.iloc[-1].Open/symDf.iloc[-2].Close
                    # spyGapRange = spyDf.iloc[-1].Open/spyDf.iloc[-2].Close
                    # qqqGapRange = qqqDf.iloc[-1].Open/qqqDf.iloc[-2].Close
                    # iwmGapRange = iwmDf.iloc[-1].Open/iwmDf.iloc[-2].Close
                    # diaGapRange = diaDf.iloc[-1].Open/diaDf.iloc[-2].Close
                    # vtiGapRange = vtiDf.iloc[-1].Open/vtiDf.iloc[-2].Close

                    # if (
                    #     gapRange < qqqGapRange * adjVal or
                    #     gapRange < spyGapRange * adjVal or
                    #     gapRange < vtiGapRange * adjVal or
                    #     gapRange < diaGapRange * adjVal or
                    #     gapRange < iwmGapRange * adjVal
                    # ): continue
                #     vwapDf = vwapDfDict["IWM"]
                ...
                # df = vwapDf[['Volume']]
                # volArr = df.values.tolist()
                # SmaVolD1 = Sma(volArr[:-10],10)

                # bias = (volArr[-1]-SmaVolD1)/SmaVolD1
                # if bias > adjVal: continue
                    ...
                    # sqqqDf = vwapDfDict["SQQQ"]
                    # mask = sqqqDf.Date < str(backtestTime.date())
                    # sqqqDf = sqqqDf.loc[mask]

                    
                    
                    # closeArr = vwapDf['Vwap'].values.tolist()
                    
                    # if len(closeArr) > 50:
                    #     SmaD1 = Sma(closeArr[-50:], 50)
                    #     bias = (vwapDf.iloc[-1].Vwap-SmaD1)/SmaD1
                    #     if bias > adjVal: continue
                ...
                #     df = df.assign(cv=(df.Close/df.C2)/(df.Volume/df.V2))

                #     avgCV = df.loc[bearCandle, 'cv'].mean()

                #     cv = (
                #         (vwapDf.iloc[-1].Close/vwapDf.iloc[-2].Close)/
                #         (vwapDf.iloc[-1].Volume/vwapDf.iloc[-2].Volume)
                #     )
                #     if cv < avgCV * adjVal: continue
                    ...
                    #     vwapDf.iloc[-3].Close) /
                    #     (vwapDf.iloc[-2].Volume /
                    #     vwapDf.iloc[-3].Volume) * adjVal
                    # ): continue

                    # qqqDf = vwapDfDict["QQQ"]
                    # mask = qqqDf.Date < str(backtestTime.date())
                    # qqqDf = qqqDf.loc[mask]
                    # qqqVolChange = qqqDf.iloc[-1].Volume/qqqDf.iloc[-2].Volume

                    # if (
                    #     qqqDf.iloc[-1].Volume <
                    #     qqqDf.iloc[-2].Volume and
                    #     qqqDf.iloc[-2].Volume <
                    #     qqqDf.iloc[-3].Volume and
                    #     qqqDf.iloc[-3].Volume <
                    #     qqqDf.iloc[-4].Volume and
                    #     qqqDf.iloc[-4].Volume <
                    #     qqqDf.iloc[-5].Volume
                    # ): continue
                    # vwapDf = vwapDfDict["QQQ"]
                    # mask = vwapDf.Date < str(backtestTime.date())
                    # vwapDf = vwapDf.loc[mask]
                    ...
                    # if (
                    #     vwapDf.iloc[-1].Volume <
                    #     vwapDf.iloc[-2].Volume * (1-adjVal) and
                    #     vwapDf.iloc[-2].Volume <
                    #     vwapDf.iloc[-3].Volume * (1+adjVal) and
                    #     vwapDf.iloc[-3].Volume >
                    #     vwapDf.iloc[-4].Volume * (1+adjVal) and
                    #     vwapDf.iloc[-4].Volume >
                    #     vwapDf.iloc[-5].Volume * (1+adjVal)
                    # ): continue
                    ...
                    # if predict < vwapDf.iloc[-1].Vwap * adjVal:
                    #     continue

                    # h1l1h2l2 h1h2<> h1c2<> h1o2<> h1l2 >h1c1h2c2<>
                    # if (
                    #     vwapDf.iloc[-1].Volume < 
                    #     vwapDf.iloc[-2].Volume
                    # ):
                    ...
                    #     df = df.assign(vw2=df.Vwap.shift(1))

                    #     df = df.assign(vw=df.Vwap/df.vw2)
                    #     avgVW = df.loc[bearCandle, 'vw'].mean()

                    #     vw = (
                    #         vwapDf.iloc[-1].Vwap/vwapDf.iloc[-2].Vwap
                    #     )
                    #     if vw > avgVW * adjVal: continue
                ...
                # # #         # if symbol not in sp500symbols: continue
                        ...
                        # if forcast < hisBarsD1[1].close * adjVal: continue
                ...
                if dateSym in dateSymProfitDict:
                    total = dateSymProfitDict[dateSym]
                else:
                    adrRange = adrDict[symbol]
                    op = trade['op']
                    sl = trade['sl']
