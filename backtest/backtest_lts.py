...
@jit(nopython=True)
def Backtest(npArr, adrRange=0.01, currency='USD', shortable=False,  basicPoint=0.01):
    ...
    for i in range(
        ...
        if (
            npArr[i][0] > npArr[i-1][3]*1.02
            # and len(positions) == 0
        ):
            # We will use average price from the current ticker
            price = math.ceil(npArr[i][0]*normalize)/normalize
            if price == 0: return 0
        ...
        if positions > 0:
            if npArr[i][2] < sl:
                ...
                price = npArr[i][3]
            price = math.floor(price*normalize)/normalize
...
def BacktestDf(df, adrRange=0.07, shortable=False, currency='USD', qqqdf=pd.DataFrame(), spydf=pd.DataFrame(), iwmdf=pd.DataFrame(), diadf=pd.DataFrame(), vtidf=pd.DataFrame(), basicPoint=0.01):
    capital = initialCapital  # initial capital
    vol = 0  # the initial amount of vol
    ...
    for i in range(
        ...
        if (
            ...
            # newdf = df.head(i-1)
            # if len(newdf) > 3:
            #     if newdf.iloc[-1].Close > newdf.iloc[-3].Open > 1.06:
            #         continue
            # if len(newdf) > 14:
            #     if newdf.iloc[-1].Close > newdf.iloc[-14].Open > 1.2:
            #         continue
            # if len(newdf) > 2:
            #     if (
            #         newdf.iloc[-2].Close >
            #         newdf.iloc[-2].Open and
            #         newdf.iloc[-1].Close <
            #         newdf.iloc[-1].Open
            #     ): continue
            # if len(newdf) > 2:
            ...
            #     if (
            #         newdf.iloc[-1].Volume >
            #         newdf.iloc[-2].Volume and
            #         newdf.iloc[-1].Close <
            #         newdf.iloc[-1].Open
            #     ): continue

            # closeArr = newdf.Close.values.tolist()
            # if len(newdf) > 25:
            #     SmaD1 = Sma(closeArr[-25:], 25)
            #     if (
            #         newdf.iloc[-1].Close >
            #         SmaD1
            #     ): continue
            ...
            # if len(newdf) > 2:
            #     SmaD1 = Sma(closeArr[-2:], 2)
            #     bias = (newdf.iloc[-1].Close-SmaD1)/SmaD1
            #     if bias > 0.03: continue
            #     if bias < -0.02: continue

            # if len(newdf) > 50:
            #     SmaD1 = Sma(closeArr[-50:], 50)
            #     bias = (newdf.iloc[-1].Close-SmaD1)/SmaD1
            #     if bias > 0.1: continue

            # if len(newdf) > 1:
            #     newdf = newdf.assign(stddev=newdf.Close.std())
            #     newdf = newdf.assign(sma=newdf.Close.mean())
            #     newdf = newdf.assign(upperBand=newdf.sma+(2 * newdf.stddev))

            #     if newdf.iloc[-1].High > newdf.iloc[-1].upperBand: continue
...
def main(currency='USD'):
    ...
    if currency=='USD':
        ...
        # qqqdf = qqqdf.assign(qqqLowRange = (qqqdf.High+qqqdf.Low)/2)
        # qqqdf = qqqdf.assign(qqqOpen = qqqdf.Open)
        # qqqdf = qqqdf.assign(qqqClose = qqqdf.Close)
        # # qqqdf = qqqdf[['qqqNextOpen','qqqLowRange']]
        # qqqdf = qqqdf[['qqqNextOpen','qqqLowRange','qqqOpen','qqqClose']]
        ...
        #     spydf = hist
        # spydf = spydf.assign(spyOpen = spydf.Open)
        # spydf = spydf.assign(spyClose = spydf.Close)
        # spydf = spydf[['spyOpen','spyClose']]

        # stockInfo = yf.Ticker("DIA")
        # hist = stockInfo.history(period="36500d")
        # hist.dropna()
        # v = hist.Volume.values
        # h = hist.High.values
        ...
        # hist['Vwap'] = Vwap(v,h,l)
        # start_path = 'mainFolder'
        # hist.to_csv('./csv/vwap/USD/DIA.csv')
        # diadf = hist
        # diadf = diadf.assign(diaOpen = diadf.Open)
        # diadf = diadf.assign(diaClose = diadf.Close)
        # diadf = diadf[['diaOpen','diaClose']]
        ...
        # hist['Vwap'] = Vwap(v,h,l)
        # start_path = 'mainFolder'
        # hist.to_csv('./csv/vwap/USD/IWM.csv')
        # iwmdf = hist
        # iwmdf = iwmdf.assign(iwmOpen = iwmdf.Open)
        # iwmdf = iwmdf.assign(iwmClose = iwmdf.Close)
        # iwmdf = iwmdf[['iwmOpen','iwmClose']]

        # stockInfo = yf.Ticker("VTI")
        # hist = stockInfo.history(period="36500d")
        # hist.dropna()
        # v = hist.Volume.values
        # h = hist.High.values
        ...
        # hist['Vwap'] = Vwap(v,h,l)
        # start_path = 'mainFolder'
        # hist.to_csv('./csv/vwap/JPY/VTI.csv')
        # vtidf = hist
        # vtidf = vtidf.assign(vtiOpen = vtidf.Open)
        # vtidf = vtidf.assign(vtiClose = vtidf.Close)
        # vtidf = vtidf[['vtiOpen','vtiClose']]
        ...
        # hist['Vwap'] = Vwap(v,h,l)
        # start_path = 'mainFolder'
        # hist.to_csv('./csv/vwap/USD/IAU.csv')
        ...
        # hist['Vwap'] = Vwap(v,h,l)
        # start_path = 'mainFolder'
        # hist.to_csv('./csv/vwap/JPY/^N225.csv')
    ...
    for symbol in profitSymList:
        ...
        #     if insiderPercent > 0.52653: continue
        ...
        # if symbol in shortableSymList:
        #     shortable = True
        capital = Backtest(npArr,adrRange,currency)
        # capital = BacktestDf(df,adrRange,currency)
        total += capital
...
def testCusAttr(currency='USD'):
    ...
    for symbol in profitSymList:
        ...
        if symbol not in attrDict:
            attrDict[symbol] =  insiderPercent
    attrList = list(attrDict.values())
