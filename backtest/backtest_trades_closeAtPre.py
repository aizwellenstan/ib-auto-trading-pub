...
try:
    ...
    for trade in trades:
        ...
        op = trade['op']
        # if(op >= 20.10 and op <= 50): continue
        sl = trade['sl']
        tp = trade['tp']
        vol = trade['vol']
        if(vol<4): continue
        ...
        #opEndTime = backtestTime+timedelta(minutes=375)
        opEndTime = backtestTime+timedelta(minutes=1400)
        ...
        closeAtPreTime = hisBarsM5[-1].date

        print(backtestTime,closeAtPreTime)
