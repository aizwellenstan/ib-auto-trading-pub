...
fillterDf = pd.read_csv (r'./csv/filterSymLst.csv', index_col=0)
fillterDf.drop
fillterJson = json.loads(fillterDf.to_json(orient = 'records'))
fillterSymLst = []
for sym in fillterJson:
    fillterSymLst.append(sym['symbol'])

df = pd.read_csv (r'./csv/sector.csv', index_col=0)
df.drop
fillterSymLst = json.loads(df.to_json(orient = 'records'))
...
try:
    ...
    if(saveQQQ):
        ...
        pickle.dump(tradeHisBarsQQQarr, open("./pickle/tradeHisBarsQQQarr.p", "wb"))
        print("pickle dump finished")
    else:
    ...
    df = pd.read_csv (r'./csv/filter_all.csv')
    df.drop
