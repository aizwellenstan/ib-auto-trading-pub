...
def plotLines(hisBars, brk :int = 2, body :bool = False, tch :int = 4, level :int = 3, lineLife :int = 4):
    ...
    try:
        ...
        if(slopeUpper > 0 and slopeLower < 0): 
...
def main():
    try:
        ...
        for trade in trades:
            ...
            # h1_morning_star_val, h1_evening_star_val, h1_engulfin_val = talib_pattern(
            #     60, backtestTime, df_h1)
            # if(h1_evening_star_val < 0):
            #     continue

            m30_morning_star_val, m30_evening_star_val, m30_engulfin_val = talib_pattern(
                30, backtestTime, df_m30)
            if(m30_evening_star_val < 0):
                continue
            # if(m30_engulfin_val < 0):
            #     buy += 1
            # if(m30_engulfin_val < 0):
            #     continue
