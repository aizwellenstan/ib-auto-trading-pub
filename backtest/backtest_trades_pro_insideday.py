...
def main():
    try:
        ...
        maxMarCapLimit = 0
        maxVolavgLimit = 0
        max_inside_day_tpval = 0

        tradeHisBarsM5arr = []
        tradeHisBarsQQQarr = []
        hisBarsD1arr = []

        inside_day_tpval = 1.1
        while inside_day_tpval <= 66.92:
            total = 0
            net = 0
            win = 0
            loss = 0
            ...
            for trade in trades:
                ...
                if(
                    ...
                    and hisBarsD1[0].open > hisBarsD1[1].low
                ...
                if inside_day:
                    tp = normalizeFloat(op+(op-sl)*inside_day_tpval,op,sl)
            ...
            if(total > maxProfit): # and winrate>0.067
                maxProfit = total
                maxTpVal = tpVal
                max_inside_day_tpval = inside_day_tpval
            print('maxTpVal',str(maxTpVal),'maxProfit',str(maxProfit))
            print('max_inside_day_tpval',str(max_inside_day_tpval))
            inside_day_tpval += 0.1
