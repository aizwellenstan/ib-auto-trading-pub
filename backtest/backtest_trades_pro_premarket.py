...
def GetLevel(price, buy, sell):
    ...
    if (buy):
        lv0 = normalizeFloat(prc+mult, 0.01)
        lv50 = normalizeFloat(prc+0.5*mult, 0.01)
    if (sell):
        lv0 = normalizeFloat(prc-mult, 0.01)
        lv50 = normalizeFloat(prc-0.5*mult, 0.01)
...
def main():
    try:
        ...
        output = open("./pickle/pro/compressed/hisBarsStocks4MPreM30arr.p", "rb")
        gc.disable()
        ...
        fee = 1.001392062 * 2
        tpVal = 3.69 #1.15137614678 #2 #5 #2 #3.19148936 #2 #15.42857143 #5.7#5.7#35.2#23.5 #25.8
        maxProfit = 0
        ...
        for trade in trades:
            ...
            backtestDate = trade['time']
            op = trade['op']
            if op<3: continue
            
            trade['result'] = ''
            trade['total'] = 0
            backtestDate = dt.strptime(backtestDate, '%Y-%m-%d')
            preTime = backtestDate + timedelta(hours = 21)
            backtestTime = backtestDate + timedelta(hours = 22) +timedelta(minutes = 30)
            ...
            if (
                ...
                if (
                    ...
                    hisBarsD1[0].open > hisBarsD1[45].high
                ): highBreak = True

                if not highBreak:
                    if (
                        hisBarsD1[1].close < hisBarsD1[180].close * 1.48
                    ): continue
            ...
            preHisBarsM30 = list(filter(lambda x:x.date >= preTime,dataM30))

            testhisBarsM30 = list(filter(lambda x:x.date >= backtestTime,preHisBarsM30))
            
            preHisBarsM30 = list(filter(lambda x:x.date < backtestTime,preHisBarsM30))

            preMaxHigh = 0
            preMinLow = 9999

            for i in preHisBarsM30:
            ...
            # op = max(testhisBarsM30[0].open,preMaxHigh + 0.01 * 3)
            # op = normalizeFloat(op, 0.01)
            op = normalizeFloat(preMaxHigh + 0.01 * 3, 0.01)
            sl = preMinLow
            if op > 16.5:
            ...
            sl = trade['sl']
            tp = normalizeFloat(op+(op-sl)*tpVal,0.01)
            vol = int((1000)/(op-sl))
            trade['vol'] = vol
