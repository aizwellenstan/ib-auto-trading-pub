...
df = pd.read_csv (r'./csv/trades_refined_2230.csv', index_col=0)
df.drop
...
def main():
    try:
        ...
        if(saveStocksD1arr):
            ...
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/hisBarsStocksFullD1arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
            df = pd.DataFrame(ignoreList)
            df.to_csv('./csv/ignore.csv')
        else:
            output = open("./pickle/pro/compressed/hisBarsStocksFullD1arr.p", "rb")
            gc.disable()
        ...
        if(saveStocksM30arr):
            ...
            pickle.dump(hisBarsStocksM30arr, open("./pickle/pro/compressed/hisBarsStocksFullM30arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
            df = pd.DataFrame(ignoreList)
            df.to_csv('./csv/ignore.csv')
        else:
            output = open("./pickle/pro/compressed/hisBarsStocksFullM30arr.p", "rb")
            gc.disable()
        ...
        for trade in trades:
            ...
            sl = trade['sl']
            sl = normalizeFloat(op*slVal,op,sl)
