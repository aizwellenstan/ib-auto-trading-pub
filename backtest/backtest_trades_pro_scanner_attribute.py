...
from modules.aiztradingview import GetProfit,GetPE,GetProfitWithADR,GetScannerWithAttribute
import requests
...
def main():
    try:
        ...
        output = open("./pickle/pro/compressed/hisBarsStocks5MD1arr.p", "rb")
        gc.disable()
        ...
        hisBarsStocksH1arr = []
        output = open("./pickle/pro/compressed/hisBarsStocks5MH1arr.p", "rb")
        gc.disable()
        ...
        for trade in trades:
            ...
            if symbol not in tradedSymList:
                tradedSymList.append(symbol)

        newAttributeValList = []
        newAttributeValList.append(attributeValList[0])
        for sym in tradedSymList:
            for s in scanner:
                if s['s'] == sym:
                    attribute = s['attribute']
                    newAttributeValList.append(attribute)
                    break
        newAttributeValList.sort()
        print(tradedSymList)
        print(newAttributeValList)

        idx = 0
        while idx < len(newAttributeValList):
            attributeLimit = newAttributeValList[idx]
            total = 0
            net = 0
            win = 0
            loss = 0
            ...
            for trade in trades:
                ...
                if attributeLimit is None: continue
                if attribute is None: continue
                if attribute < attributeLimit: continue
                ...
                sl = normalizeFloat(op - adrRange * 0.03, 0.01, 0.01)
                ...
                if (
                    hisBarsD1[7].close > hisBarsD1[7].open and
                    hisBarsD1[5].close < hisBarsD1[5].open and
                    hisBarsD1[6].open < hisBarsD1[7].low and
                    (
                        hisBarsD1[6].open - hisBarsD1[6].close
                        > hisBarsD1[1].close * 0.05
                    )
                ): continue
