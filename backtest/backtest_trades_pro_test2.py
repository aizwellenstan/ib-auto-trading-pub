...
df = pd.read_csv (r'./csv/trades2.csv', index_col=0)
df.drop
...
fillterDf = pd.read_csv (r'./csv/loweps.csv', index_col=0)
fillterDf.drop
fillterSymLst = json.loads(fillterDf.to_json(orient = 'records'))
...
try:
    ...
    for trade in trades:
        ...
        if symbol in fillter_list: continue
        ...
        dataH4 = []
        for hisBarsStockH4 in hisBarsStocksH4arr:
            if symbol == hisBarsStockH4['s']:
                dataH4 = hisBarsStockH4['d']
                break
        if(len(dataH4) < 6):
            print(symbol,'H4 no data')
            continue
        hisBarsH4 = list(filter(lambda x:x.date <= backtestTime,dataH4))
        hisBarsH4 = hisBarsH4[::-1]

        dataH3 = []
        for hisBarsStockH3 in hisBarsStocksH3arr:
            if symbol == hisBarsStockH3['s']:
                dataH3 = hisBarsStockH3['d']
                break
        if(len(dataH3) < 6):
            print(symbol,'H3 no data')
            continue
        hisBarsH3 = list(filter(lambda x:x.date <= backtestTime,dataH3))
        hisBarsH3 = hisBarsH3[::-1]

        dataH2 = []
        for hisBarsStockH2 in hisBarsStocksH2arr:
            if symbol == hisBarsStockH2['s']:
                dataH2 = hisBarsStockH2['d']
                break
        if(len(dataH2) < 6):
            print(symbol,'H2 no data')
            continue
        hisBarsH2 = list(filter(lambda x:x.date <= backtestTime,dataH2))
        hisBarsH2 = hisBarsH2[::-1]

        # dataH1 = []
        # for hisBarsStockH1 in hisBarsStocksH1arr:
        #     if symbol == hisBarsStockH1['symbol']:
        #         dataH1 = hisBarsStockH1['data']
        #         break

        # if(len(dataH1) < 6):continue
        # hisBarsH1 = list(filter(lambda x:x.date <= backtestTime.date(),dataH1))
        ...
        if(
            ...
            # and hisBarsD1[1].high - hisBarsD1[1].low
            #     < hisBarsD1[2].high - hisBarsD1[2].low
            # and hisBarsD1[1].low > hisBarsD1[2].low
            hisBarsD1[1].high < hisBarsD1[2].high
            and hisBarsD1[1].high - hisBarsD1[1].low
                < (hisBarsD1[2].close - hisBarsD1[2].open) * 0.45
        ):
            buy += 1

        if(
            # hisBarsH8[2].close > hisBarsH8[2].open
            # and hisBarsH8[2].close - hisBarsH8[2].open
            #     > hisBarsH8[3].high - hisBarsH8[3].low
            # and hisBarsH8[1].high - hisBarsH8[1].low
            #     < hisBarsH8[2].high - hisBarsH8[2].low
            # and hisBarsH8[1].low > hisBarsH8[2].low
            hisBarsH8[1].high < hisBarsH8[2].high
            and hisBarsH8[1].high - hisBarsH8[1].low
                < (hisBarsH8[2].close - hisBarsH8[2].open) * 0.45
        ):
            buy += 1

        if(
            # hisBarsH4[2].close > hisBarsH4[2].open
            # and hisBarsH4[2].close - hisBarsH4[2].open
            #     > hisBarsH4[3].high - hisBarsH4[3].low
            # and hisBarsH4[1].high - hisBarsH4[1].low
            #     < hisBarsH4[2].high - hisBarsH4[2].low
            # and hisBarsH4[1].low > hisBarsH4[2].low
            hisBarsH4[1].high < hisBarsH4[2].high
            and hisBarsH4[1].high - hisBarsH4[1].low
                < (hisBarsH4[2].close - hisBarsH4[2].open) * 0.45
        ):
            buy += 1

        if(
            # hisBarsH3[2].close > hisBarsH3[2].open
            hisBarsH3[2].close - hisBarsH3[2].open
            > hisBarsH3[3].high - hisBarsH3[3].low
            # and hisBarsH3[1].high - hisBarsH3[1].low
            #     < hisBarsH3[2].high - hisBarsH3[2].low
            # and hisBarsH3[1].low > hisBarsH3[2].low
            and hisBarsH3[1].high < hisBarsH3[2].high
            and hisBarsH3[1].high - hisBarsH3[1].low
                < (hisBarsH3[2].close - hisBarsH3[2].open) * 0.45
        ):
            buy += 1

        if(
            # hisBarsH2[2].close > hisBarsH2[2].open
            hisBarsH2[2].close - hisBarsH2[2].open
            > hisBarsH2[3].high - hisBarsH2[3].low
            # and hisBarsH2[1].high - hisBarsH2[1].low
            #     < hisBarsH2[2].high - hisBarsH2[2].low
            # and hisBarsH2[1].low > hisBarsH2[2].low
            and hisBarsH2[1].high < hisBarsH2[2].high
            and hisBarsH2[1].high - hisBarsH2[1].low
                < (hisBarsH2[2].close - hisBarsH2[2].open) * 0.45
        ):
            buy += 1
        ...
        # if(
        #     hisBarsM30[2].high < hisBarsM30[3].low
        #     and hisBarsM30[1].low > hisBarsM30[2].high
        # ):
        #     buy += 1

        # if(
        #     hisBarsH1[2].high < hisBarsH1[3].low
        #     and hisBarsH1[1].low > hisBarsH1[2].high
        # ):
        #     buy += 1
        ...
        #         > (hisBarsD1[2].high - hisBarsD1[2].low) * 0.5
        # ):
        #     buy += 1
        #     buysSetup = True
        ...
        # # Relative strength
        # if(
        #     hisBarsD1[0].open/hisBarsD1[11].open
        #     > hisBarsQQQD1[0].open/hisBarsQQQD1[11].open
        # ):
        #     buy +=1 
        ...
        # if(
        #     hisBarsM30[3].close > hisBarsM30[3].open
        ...
        #     and hisBarsM30[3].close - hisBarsM30[3].open
        #         > (hisBarsM30[4].high - hisBarsM30[4].low)
        # ):  
        #     buy += 1
        #     fourBarPlay = True
        ...
        if buy < 1: continue
        eod = False
        if buysSetup and buy==1:
            tp = normalizeFloat(op+(op-sl)*1.76901408451,op,sl)
            eod = True
        if fourBarPlay and buy==1:
            tp = normalizeFloat(op+(op-sl)*1.33333333,op,sl)
            eod = True
        ...
        if three_bar_play and buy == 1:
            op = normalizeFloat(cus_op,op,sl)
            tp = normalizeFloat(op+(op-sl)*20.4,op,sl)
