...
try:
    ...
    if(saveSPY):
        for trade in trades:
            ...
            hisBarsSPY = ib.reqHistoricalData(
                contractSPY, endDateTime=backtestTime, durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)

            while(len(hisBarsSPY)<6):
                print("timeout")
                hisBarsSPY = ib.reqHistoricalData(
                contractSPY, endDateTime=backtestTime, durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)

            hisBarsSPY = hisBarsSPY[::-1]
            print(backtestTime,hisBarsSPY[0].date)
            hisBarsSPYD1arr.append(hisBarsSPY)
            print("data get")

        pickle.dump(hisBarsSPYD1arr, open("./pickle/hisBarsSPYD1arr.p", "wb"))
        print("pickle dump finished")
    else:
        hisBarsSPYD1arr = pickle.load(open("./pickle/hisBarsSPYD1arr.p", "rb"))
        print(hisBarsSPYD1arr[0][0].date)
    ...
    for trade in trades:
        ...
        hisBarsSPYD1 = hisBarsSPYD1arr[a-1]
        ...
        # if(
        #     hisBarsSPYD1[1].close < hisBarsSPYD1[1].open
        #     and hisBarsSPYD1[0].open > hisBarsSPYD1[1].low
        #     and hisBarsSPYD1[0].open < hisBarsSPYD1[1].high
        #     and hisBarsSPYD1[0].open 
        #         < hisBarsSPYD1[1].low + (hisBarsSPYD1[1].high
        #                                     -hisBarsSPYD1[1].low) * 0.88
        # ): continue
        ...
        if(
            (hisBarsH2[1].high < hisBarsH2[2].high
            and hisBarsH2[1].low > hisBarsH2[2].low)
        ): continue

        # print(hisBarsH2[1].date, hisBarsH2[2].date)
