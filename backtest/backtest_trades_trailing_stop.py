...
try:
    ...
    while(moveSlDevide <= 10000):
                ...
                for i in hisBarsM5:
                    if i.date >= triggeredTime:
                        if i.date == triggeredTime:
                            ...
                            if i.date == triggeredTime+timedelta(minutes=trailSlTime):
                                trailPrice = i.low-0.01
                                if(trailPrice>sl):
                                    sl = op+(i.low - op)/moveSlDevide
                                    print(i.date)
                            if i.low <= sl:
                                net = (sl-op)*vol - fee
                                trade['total'] = net
        ...
        if(total > maxProfit): # and winrate>0.067
            ...
            maxMoveSlMoveSlDevide = moveSlDevide
        ...
        print('maxMoveSlMoveSlDevide ',str(maxMoveSlMoveSlDevide))
        moveSlDevide += 1
