...
efficiencyPath = f"{rootPath}/backtest/pickle/pro/compressed/efficiency.p"
quarterPath = f"{rootPath}/backtest/pickle/pro/compressed/quarter.p"
productivityPath = f"{rootPath}/backtest/pickle/pro/compressed/productivity.p"
newsPath = f"{rootPath}/backtest/pickle/pro/compressed/news.p"
perPath = f"{rootPath}/backtest/pickle/pro/compressed/dataDictPerJP.p"
...
efficiencyDict = LoadPickle(efficiencyPath)
quarterDict = LoadPickle(quarterPath)
productivityDict = LoadPickle(productivityPath)
newsDict = LoadPickle(newsPath)
perDict = LoadPickle(perPath)
...
def Backtest(dataDict, length, valuepegDict, gyoushuDict, pegDict, newsDict, perDict):
    balance = 1
    smaDict = {}
    sma18Dict = {}
    sma4Dict = {}
...
'金属製品', '紙・パルプ', '倉庫・運輸', '水産・農林']
    ...
    for symbol, per in perDict.items():
        if len(per[0]) < 11: 
            perIgnoreList.append(symbol)
            continue
        newPerDict[symbol] = {item[0]: [item[9], item[10]] for item in per}
    for i in range(460, length):
        topPerf = 0
        topSymbol = ""
        today = sampleArrJP[i][5]
        print(today,i)
        # yesterday = sampleArrJP[i-1][5]
        for symbol, npArr in dataDict.items():
            if symbol in perIgnoreList: continue
            # if yesterday not in newsDict: continue
            # news = newsDict[yesterday]
            # newsUp = news[0]
            # if len(newsUp) < 1: continue
            # # if symbol not in newsUp: continue
            # newsDwn = news[1]
            # if symbol in newsDwn: continue
            # peg = pegDict[symbol]
            # idx = -1
            # pegLength = len(peg)
            ...
            if symbol in newPerDict:
                if npArr[i-1][5] in newPerDict[symbol]:
                    # per = newPerDict[symbol][npArr[i-1][5]][0]
                    pbr = newPerDict[symbol][npArr[i-1][5]][1]
                    # if pbr > 22.63: continue
                    # if per > 117.17: continue
...
for symbol, npArr in dataDict.items():
    ...
    junnrieki = pl[junnriekiIdx]
    junnrieki2 = pl2[junnriekiIdx]
    junnrieki3 = pl3[junnriekiIdx]

    # eps = pl[epsIdx]
    # if eps == "-": continue
    # if eps == "赤字": continue
    # if eps < -96.6: continue
    ...
    freecf = cf[4]
    if freecf < -31700000000: continue
    ...
    if symbol not in efficiencyDict: continue
    efficiency = float(efficiencyDict[symbol][0][-2][4])
    if (
        float(efficiencyDict[symbol][0][-3][4]) < 
        float(efficiencyDict[symbol][0][-4][4]) and
        float(efficiencyDict[symbol][0][-2][4]) < 
        float(efficiencyDict[symbol][0][-3][4])
    ): continue

    efficiency = efficiencyDict[symbol]
    kaitenkikann = efficiency[1]
    uriagesaikennkaiten = kaitenkikann[-1][1]
    shiiresaimukaiten = kaitenkikann[-1][2]
    tanaoroshishisann = kaitenkikann[-1][3]
    ccc = kaitenkikann[-1][4]

    if symbol not in quarterDict: continue
    qOnQ = quarterDict[symbol][0]

    shinnchokuritsu = quarterDict[symbol][1]
    shinnchokuritsuLen = len(shinnchokuritsu)
    if shinnchokuritsuLen == 4:
        keijyouriekishinnchoku = shinnchokuritsu[2]
        keijyouriekishinnchoku1q = keijyouriekishinnchoku[0]
        keijyouriekishinnchoku2q = keijyouriekishinnchoku[1]
        keijyouriekishinnchoku3q = keijyouriekishinnchoku[2]
        # if keijyouriekishinnchoku1q != "-":
        #     if "%" not in str(keijyouriekishinnchoku1q):
        #         if keijyouriekishinnchoku1q < 0.25: continue
        # if keijyouriekishinnchoku2q != "-":
        #     if "%" not in str(keijyouriekishinnchoku2q):
        #         if keijyouriekishinnchoku2q < 0.3694: continue
        if keijyouriekishinnchoku3q != "-":
            if "%" not in str(keijyouriekishinnchoku3q):
                if keijyouriekishinnchoku3q < 0.7534: continue
    

        # print(quarter)
    # if symbol not in productivityDict: continue
    # productivity = productivityDict[symbol][0][-1]
    # productivity2 = productivityDict[symbol][0][-2]
    # if productivity[2] < productivity2[2]: continue
...
Backtest(cleanDataDict, length, valuepegDict, gyoushuDict, pegDict, newsDict, perDict)
