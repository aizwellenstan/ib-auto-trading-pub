...
valuePath = f"{rootPath}/backtest/pickle/pro/compressed/value.p"
...
valueDict = LoadPickle(valuePath)
...
def Backtest(dataDict, length, valueDict, valLimit):
    balance = 1
    smaDict = {}
    sma3Dict = {}
    ignoreDict = {}
    for i in range(460, length):
        ...
        for symbol, npArr in dataDict.items():
            value = valueDict[symbol][-1][3]
            if value == "-" or "inf" in str(value):
                value = valueDict[symbol][-2][3]
            if npArr[i-1][3] > value*valLimit: continue
...
for symbol, npArr in dataDict.items():
    ...
    if symbol not in valueDict: continue
    if valueDict[symbol][-2][3] == "-": continue
    ...
    # continue
    # if plLength not in [11, 12]: continue
    junnriekiIdx = 4
    epsIdx = 6
    roeIdx = 7
    roaIdx = 8
    ...
    # if junnrieki < -302000000: continue
    ...
    #     if keijyourieki < -250000000: continue
    ...
    if bsLength in [8, 9]:
        ...
        # if yuurishifusaihiritsu > 371.93: continue
    ...
    if eigyoucfmargin < -12.91: continue
    ...
    if dividendLength in [6, 8, 9, 10]:
        ...
        # if soukanngenngaku != "-": 
        #     if soukanngenngaku < 1300: continue

        soukanngennseikou = dividend[soukanngennseikouIdx]
        if soukanngennseikou == "赤字": continue
        # if (
        #     soukanngennseikou != "-" and
        #     soukanngennseikou != 0
        # ):
        #     if soukanngennseikou < 8.7: continue
        #     if soukanngennseikou > 99.8: continue
    ...
    if dividendLength in [3, 6, 7, 9, 10]:
        ...
        # if shihyourimawari > 153.4: continue
...
# balance = Backtest(cleanDataDict, length, valueDict, valLimit)

def HandleBacktest(cleanDataDict, length, valueDict, valLimit):
    balance = Backtest(cleanDataDict, length, valueDict, valLimit)
    return [valLimit, balance]
...
maxValLimit = 0
maxBalance = 1
valLimitList = [i/10 for i in range(2000)]
valLimitList.sort(reverse=True)
with ThreadPoolExecutor(max_workers=100) as executor:
    futures = [executor.submit(HandleBacktest, cleanDataDict, length, valueDict, valLimit) for valLimit in valLimitList]
    for future in as_completed(futures):
        ...
        if balance > maxBalance:
            maxBalance = balance
            maxValLimit = valLimit
            print(maxValLimit, maxBalance)