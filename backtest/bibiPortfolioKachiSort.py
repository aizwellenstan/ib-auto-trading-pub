...
def Backtest(dataDict, length, valueDict):
    balance = 1
    smaDict = {}
    sma3Dict = {}
    ignoreDict = {}
    for i in range(460, length):
        ...
        for symbol, npArr in dataDict.items():
            if npArr[i-1][3] > valueDict[symbol][-1][4]*0.9: continue
            ...
            perf = valueDict[symbol][-1][4] / npArr[i-1][3]
            if perf > topPerf:
                topPerf = perf
                topSymbol = symbol
...
Backtest(cleanDataDict, length, valueDict)
