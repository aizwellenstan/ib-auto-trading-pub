...
from modules.aiztradingview import GetCloseJP, GetCashUS, GetClose
...
def main(update=False):
    ...
    cashDict = {}
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(CheckCash, symbol, cashDictUS) for symbol in list(cashDictUS.keys())]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
