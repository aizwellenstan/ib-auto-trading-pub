...
from modules.financials import GetOperatingInvestmentCashFlow
...
def HandleetOperatingInvestmentCashFlow(symbol):
    operating, investement = GetOperatingInvestmentCashFlow(symbol)
    return [symbol, operating, investement]
...
def UpdateData(symbolList):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleetOperatingInvestmentCashFlow, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            operating = result[1]
            investement = result[2]
            if len(operating) > 0:
                operatingDict[symbol] = operating
            if len(investement) > 0:
                investementDict[symbol] = investement
    return operatingDict, investementDict
...
def main(update=False):
    ...
    if update:
        operatingDict, investementDict = UpdateData(symbolListJP)
        ...
        pickle.dump(operatingDict, open(operatingPath, "wb"))
        pickle.dump(investementDict, open(investementPath, "wb"))
