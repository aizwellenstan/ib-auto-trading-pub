...
from modules.strategy.copula import Copula, HpCopula
...
JP_START = time_to_minutes("10:00")
JP_END = time_to_minutes("10:30")
...
for symbol in symbolList:
    ...
    # contractMBT = Future(conId=699802811, symbol='MBT', lastTradeDateOrContractMonth='20241025', multiplier='0.1', currency='USD', localSymbol='MBTV4', tradingClass='MBT')
    # ib.qualifyContracts(contractMBT)
    ...
    # contractMCL = Future(conId=660309062, symbol='MCL', lastTradeDateOrContractMonth='20241021', multiplier='100', currency='USD', localSymbol='MCLX4', tradingClass='MCL')
    # ib.qualifyContracts(contractMCL)
    dfTarget = df
    ...
    for symbol in cohort:
        ...
        # contract = Stock(symbol, 'TSEJ', 'JPY')
        # ib.qualifyContracts(contract)
        df = ibc.GetDataDf(contractDict["NQ"], tf)
        # df = df[df['volume'] >= 100]
        df = df[df['date'].isin(dfTarget['date'])]
        dfTarget = dfTarget.join(df, lsuffix='', rsuffix=f'_{symbol}')
    dfTarget = dfTarget.dropna()
    # dfTarget = dfTarget[dfTarget[f'volume_{symbol}'] >= 1]

    df['Open'] = dfTarget['open']
    df['High'] = dfTarget['high']
    df['Low'] = dfTarget['low']
    df['Close'] = dfTarget['close']
    df['Volume'] = dfTarget['volume']
    df['Date'] = dfTarget['date']

    oriNpArr = df[['Open', 'High', 'Low', 'Close', 'Volume', 'barCount', 'Date']].to_numpy()
    print(oriNpArr)
    npArrDict = {}
    for symbol in cohort:
        npArr = dfTarget[[f'open_{symbol}',f'high_{symbol}',f'low_{symbol}',f'close_{symbol}',f'volume_{symbol}',f'barCount_{symbol}']].to_numpy()
        npArrDict[symbol] = npArr
    print(npArrDict)
    TotalSignal = []
    SlSignal = []
    TpSignal = []
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        npArrTQQQ = npArrDict[pairSymbol][:i+2]
        signal, op, sl, tp = HpCopula(npArr, npArrTQQQ, tf, tick_val)
