...
from modules.rironkabuka import GetRironFinancial, GetDekidakakaitennritsu
...
def HandleGetDekidakakaitennritsu(symbol):
    dekidakakaitennritsu = GetDekidakakaitennritsu(symbol)
    return [symbol, dekidakakaitennritsu]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, dekidakakaitennritsuDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetDekidakakaitennritsu, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            dekidakakaitennritsu = result[1]
            if dekidakakaitennritsu <= -6: continue
            dekidakakaitennritsuDict[symbol] = dekidakakaitennritsu
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, dekidakakaitennritsuDict, zandakaDict
...
def main(update=False):
    ...
    dekidakakaitennritsuPath = f"{rootPath}/backtest/pickle/pro/compressed/dekidakakaitennritsu.p"
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, dekidakakaitennritsuDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(dekidakakaitennritsuDict, open(dekidakakaitennritsuPath, "wb"))
