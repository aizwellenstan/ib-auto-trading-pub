...
def main():
    try:
        ...
        # saveQQQD1 = True
        # if(saveQQQD1):
        #     contractQQQ = Stock("QQQ", 'SMART', 'JPY')
        ...
        if(saveStocksD1Dict):
            for symbol in filter_sym_list:
                ...
                if len(hisBarsD1) > 0:
                    hisBarsStocksD1Dict[symbol] = hisBarsD1
                else:
                    ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1Dict, open("./pickle/pro/compressed/jp/hisBarsStocks8MD1arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
            df = pd.DataFrame(ignoreList)
            df.to_csv('./csv/ignoreD1.csv')
        else:
            output = open("./pickle/pro/compressed/jp/hisBarsStocks8MD1arr.p", "rb")
            gc.disable()
            ...
            for hisBarsStockD1 in hisBarsStocksD1Arr:
                hisBarsStocksD1Dict[hisBarsStockD1['s']] = hisBarsStockD1['d']
            pickle.dump(hisBarsStocksD1Dict, open("./pickle/pro/compressed/jp/hisBarsStocks8MD1Dict.p", "wb"),protocol=-1)
            print("pickle dump finished")
        ...
        hisBarsStocksH2Dict = {}
        saveStocksH2Dict = True
        if(saveStocksH2Dict):
            for symbol in filter_sym_list:
                contract = Stock(symbol, 'SMART', 'JPY')
                hisBarsH2 = ib.reqHistoricalData(
                    contract, endDateTime='', durationStr='240 D',
                    barSizeSetting='2 hours', whatToShow='ASK', useRTH=True)
                maxTrys = 0
                while(len(hisBarsH2)<6 and maxTrys<=20):
                    print("timeout")
                    hisBarsH2 = ib.reqHistoricalData(
                        contract, endDateTime='', durationStr='210 D',
                        barSizeSetting='2 hours', whatToShow='ASK', useRTH=True)
                    maxTrys += 1
                hisBarsStocksH2Dict[symbol] = hisBarsH2
            pickle.dump(hisBarsStocksH2Dict, open("./pickle/pro/compressed/jp/hisBarsStocks8MH2arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
        else:
            output = open("./pickle/pro/compressed/jp/hisBarsStocks8MH2arr.p", "rb")
            gc.disable()
            ...
            for hisBarsStockH2 in hisBarsStocksH2Arr:
                hisBarsStocksH2Dict[hisBarsStockH2['s']] = hisBarsStockH2['d']
            pickle.dump(hisBarsStocksH2Dict, open("./pickle/pro/compressed/jp/hisBarsStocks8MH2Dict.p", "wb"),protocol=-1)
            print("pickle dump finished")

        # hisBarsStocksH1Dict = {}
        # saveStocksH1Dict = True
        # if(saveStocksH1Dict):
        #     for symbol in filter_sym_list:
        #         contract = Stock(symbol, 'SMART', 'JPY')
        ...
        #                 barSizeSetting='1 hour', whatToShow='ASK', useRTH=True)
        #             maxTrys += 1
        #         hisBarsStocksH1Dict[symbol] = hisBarsH1
        #     pickle.dump(hisBarsStocksH1Dict, open("./pickle/pro/compressed/jp/hisBarsStocks8MH1arr.p", "wb"),protocol=-1)
        #     print("pickle dump finished")
        # else:
        #     output = open("./pickle/pro/compressed/jp/hisBarsStocks8MH1arr.p", "rb")
        #     gc.disable()
        ...
        #         hisBarsStocksH1Dict[hisBarsStockH1['s']] = hisBarsStockH1['d']
        #     pickle.dump(hisBarsStocksH1Dict, open("./pickle/pro/compressed/jp/hisBarsStocks8MH1Dict.p", "wb"),protocol=-1)
        #     print("pickle dump finished")
