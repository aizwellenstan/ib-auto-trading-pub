...
# TradingView
fillterDf = pd.read_csv (r'./csv/livetradersProfitSym.csv', index_col=0)
fillterDf.drop
    ...
    try:
        ...
        if(saveStocksD1arr):
            for symbol in filter_sym_list:
                ...
                if len(hisBarsD1) > 0:
                    hisBarsStocksD1arr.append({'s': symbol,'d': hisBarsD1})
                else:
                    ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/hisBarsStocksLT4Marr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
        ...
        if(saveStocksM30arr):
            ...
            pickle.dump(hisBarsStocksM30arr, open("./pickle/pro/compressed/hisBarsStocksLT4MM30arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
            df = pd.DataFrame(ignoreList)
        ...
        if(saveStocksM1arr):
            for symbol in filter_sym_list:
                ...
                maxTrys = 0
                while(len(hisBarsM1)<6 and maxTrys<=20):
                    print("timeout")
                    hisBarsM1 = ib.reqHistoricalData(
                    contract, endDateTime='', durationStr='120 D',
                    barSizeSetting='1 min', whatToShow='ASK', useRTH=True)
            ...
            pickle.dump(hisBarsStocksM1arr, open("./pickle/pro/compressed/hisBarsStocksLT4MM1arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
            df = pd.DataFrame(ignoreList)
            df.to_csv('./csv/ignoreM1.csv')
        else:
            output = open("./pickle/pro/compressed/hisBarsStocksRFM1arr.p", "rb")
            gc.disable()
