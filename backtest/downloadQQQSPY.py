...
from modules.aiztradingview import GetProfit
# import requests
# import talib
...
# TradingView
# fillterDf = pd.read_csv (r'./csv/trades_4M.csv', index_col=0)
# fillterDf.drop
...
# fillterDf = pd.read_csv (r'./csv/livetradersSym.csv')
# fillterDf.drop
# filter_symbols = json.loads(fillterDf.to_json(orient = 'records'))
# filter_sym_list = []
# for i in filter_symbols:
...
def main():
    try:
        ...
        # saveQQQD1 = True
        # if(saveQQQD1):
        #     contracXLE = Stock("QQQ", 'SMART', 'USD')
        #     hisBarsQQQD1 = ib.reqHistoricalData(
        #         contracXLE, endDateTime='', durationStr='365 D',
        #         barSizeSetting='1 day', whatToShow='ASK', useRTH=True)

        #     while(len(hisBarsQQQD1)<6):
        #         print("timeout")
        #         hisBarsQQQD1 = ib.reqHistoricalData(
        #             contracXLE, endDateTime=backtestTime, durationStr='365 D',
        #             barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
        ...
        #         contracXLE, endDateTime='', durationStr='90 D',
        ...
        #         print("timeout")
        #         hisBarsQQQM5 = ib.reqHistoricalData(
        #             contracXLE, endDateTime=backtestTime, durationStr='90 D',
        #             barSizeSetting='5 mins', whatToShow='ASK', useRTH=True)
        ...
        if(saveStocksD1arr):
            ...
            if len(hisBarsD1) > 0:
                hisBarsStocksD1arr=hisBarsD1
            else:
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/QQQ8MD1arr.p", "wb"),protocol=-1)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/SPY8MD1arr.p", "wb"),protocol=-1)

            contract = Stock('VTI', 'SMART', 'USD')
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/VTI8MD1arr.p", "wb"),protocol=-1)

            print("pickle dump finished")

            contract = Stock('DIA', 'SMART', 'USD')
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/DIA8MD1arr.p", "wb"),protocol=-1)

            print("pickle dump finished")

            contract = Stock('IWM', 'SMART', 'USD')
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/IWM8MD1arr.p", "wb"),protocol=-1)

            print("pickle dump finished")

            contract = Stock('TLT', 'SMART', 'USD')
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/TLT8MD1arr.p", "wb"),protocol=-1)

            # contract = Stock('XLF', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLF8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('EEM', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/EEM8MD1arr.p", "wb"),protocol=-1)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLE8MD1arr.p", "wb"),protocol=-1)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLU8MD1arr.p", "wb"),protocol=-1)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/SOXL8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            contract = Stock('XLI', 'SMART', 'USD')
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLI8MD1arr.p", "wb"),protocol=-1)

            print("pickle dump finished")

            # contract = Stock('SOXS', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/SOXS8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('XLV', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLV8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('TZA', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/TZA8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('XLP', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLP8MD1arr.p", "wb"),protocol=-1)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/TNA8MD1arr.p", "wb"),protocol=-1)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/IAU8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('KRE', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/KRE8MD1arr.p", "wb"),protocol=-1)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLK8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('IEF', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/IEF8MD1arr.p", "wb"),protocol=-1)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XOP8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('IEFA', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/IEFA8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('VEA', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/VEA8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            # contract = Stock('IYR', 'SMART', 'USD')
            # hisBarsD1 = ib.reqHistoricalData(
            #     contract, endDateTime='', durationStr='365 D',
            #     barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            # else:
            #     ignoreList.append(symbol)
            # pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/IYR8MD1arr.p", "wb"),protocol=-1)

            # print("pickle dump finished")

            contract = Stock('XLB', 'SMART', 'USD')
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='365 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if len(hisBarsD1) > 0:
                ...
                ignoreList.append(symbol)
            pickle.dump(hisBarsStocksD1arr, open("./pickle/pro/compressed/XLB8MD1arr.p", "wb"),protocol=-1)
        ...
        # symbolLst = []
        # for sym in hisBarsStocksD1arr:
        #     s = sym["s"]
        #     symbolLst.append({"symbol":s})
        # df = pd.DataFrame(symbolLst)
        # df.to_csv('./csv/refinedSym.csv')
        ...
        if(saveStocksH1arr):
            ...
            pickle.dump(hisBarsStocksH1arr, open("./pickle/pro/compressed/hisBarsStocks8MH1arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
        else:
        ...
        if(saveStocksM30arr):
            ...
            pickle.dump(hisBarsStocksM30arr, open("./pickle/pro/compressed/hisBarsStocks4MPreM30arr.p", "wb"),protocol=-1)
            print("pickle dump finished")
            print(ignoreList)
            df = pd.DataFrame(ignoreList)
            df.to_csv('./csv/ignoreM30.csv')
        else:
        ...
        for trade in trades:
            ...
            # if(d1_evening_star_val < 0):    continue
            ...
            if inside_day:
                tp = normalizeFloat(op+(op-sl)*31.9,op,sl)
            if bias_over:
                if (
                    hisBarsD1[0].open / hisBarsD1[1].close>1.06
                ):
                    tp = normalizeFloat(op+(op-sl)*3.5,op,sl)
                else:
                    tp = normalizeFloat(op+(op-sl)*10.4,op,sl)
