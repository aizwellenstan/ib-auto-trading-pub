...
from modules.irbank import GetZandaka, GetDividend, GetEfficiency
...
def HandleGetEfficiency(symbol):
    efficiency = GetEfficiency(symbol)
    return [symbol, efficiency]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, efficiencyDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetEfficiency, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            efficiency = result[1]
            if len(efficiency) < 2: continue
            efficiencyDict[symbol] = efficiency
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, efficiencyDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, efficiencyDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(efficiencyDict, open(efficiencyPath, "wb"))
