...
JP_START = time_to_minutes("09:30")
...
tradable = []
all = []
# df = pd.read_csv("cfd_threeBarPlay_us.csv")
# df.columns = ["i","symbol"]
...
for symbol in symbolList:
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = EmaCross(npArr, tf, 1)
    ...
    if sortino > 122 and profitFactor > 1.6 and returns_ann > 30:
        ...
        df.to_csv('cfd_emaCross_us.csv')

    all.append([symbol, returns, sharpe, sortino, calmar, returns_ann, profitFactor])
    df = pd.DataFrame(all)
    df.columns = ["symbol", "returns", "sharpe", "sortino", "calmar", "returns_ann", "profitFactor"]
    df.to_csv('emaCross_us.csv')