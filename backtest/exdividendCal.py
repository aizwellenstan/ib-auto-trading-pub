...
def GetDividendCalanderDict(update=False):
    picklePath = "./pickle/pro/compressed/dividendCalendar3.p"
    ...
    while shift < 360:
        ...
        while val > 0.8:
                ...
                for exDivDate in dividendCalendarList:
                    if exDivDate < date:
                        ...
                        for i in exDivList:
                            ...
                            length = min(1076, len(npArr))
                            npArr = npArr[-length:]
                            op = 0
                            close = 0
                            div = 0
                            for j in range(1, length):
                                if npArr[j][5] < buyDate:
                                    op = 0
                                    # if (
                                    #     npArr[j][0] < npArr[j-1][3]
                                    # ):
                                    op = npArr[j][0] * val # close
                                    if npArr[j][2] > op:
                                        op = 0
                                elif npArr[j][5] == buyDate:
                                    close = npArr[j][0]
                                    div = npArr[j][4]
                                    break
                            if op == 0:
            ...
            if balance > maxBalance:
                ...
                maxVal = val
                maxShift = shift
                print(maxBalance, maxVal, maxShift)
            val -= 0.001
        shift += 1
    return balance
...
print(dividendCalendarDict.keys())
