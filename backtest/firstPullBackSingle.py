...
from modules.strategy.firstPullBack import FirstPullBack
...
for symbol in symbolList:
    try:
        ...
        # ib.qualifyContracts(contract)
        print(tf, contractMES.symbol)
        df = ibc.GetDataDf(contractGC, tf)
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = FirstPullBack(npArr, tf)
