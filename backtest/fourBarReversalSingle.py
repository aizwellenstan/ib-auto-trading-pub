...
from modules.strategy.fourBarReversal import FourBarReversal
...
for symbol in symbolList:
    ...
    oriNpArr = df[['Open', 'High', 'Low', 'Close', 'Volume', 'Date']].to_numpy()
    TotalSignal = []
    SlSignal = []
    TpSignal = []
    trades = []
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = FourBarReversal(npArr, tf, tick_val)
