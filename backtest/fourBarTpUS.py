...
indexHoldings = GetIndexHoldings()
liquidEtfs = GetLiquidETF()
print(indexHoldings)
symbolList = liquidEtfs
...
for symbol in symbolList:
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = FourBarTP(npArr, tf, 0.01)
    ...
    if (
        ...
        tradable.append([symbol, returns, sharpe, sortino, calmar, returns_ann, profitFactor, sqn])
        df = pd.DataFrame(tradable)
        df.columns = ["symbol", "returns", "sharpe", "sortino", "calmar", "returns_ann", "profitFactor", "sqn"]
        df.to_csv('cfd_fourBarTp_us.csv')

    allSymbol.append([symbol, returns, sharpe, sortino, calmar, returns_ann, profitFactor, sqn])
    df = pd.DataFrame(allSymbol)
    df.columns = ["symbol", "returns", "sharpe", "sortino", "calmar", "returns_ann", "profitFactor", "sqn"]
    df.to_csv('fourBarTp_us.csv')