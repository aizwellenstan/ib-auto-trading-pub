...
from modules.financials import GetFreeCashFlow
...
def UpdateData(symbolList):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        ...
        for future in as_completed(futures):
            ...
            symbol = result[0]
            freeCashFlow = result[1]
            if len(freeCashFlow) > 0:
                freeCashFlowDict[symbol] = freeCashFlow
    return freeCashFlowDict
...
def main(update=False):
    ...
    if update:
        freeCashFlowDict = UpdateData(symbolListJP)
        ...
        pickle.dump(freeCashFlowDict, open(freeCashFlowPath, "wb"))
