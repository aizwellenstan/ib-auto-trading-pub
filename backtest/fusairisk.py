...
from modules.rironkabuka import GetRironFinancial, GetFusairisk
...
def HandleGetFusairisk(symbol):
    fusairisk = GetFusairisk(symbol)
    return [symbol, fusairisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, fusairiskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetFusairisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            fusairisk = result[1]
            if fusairisk < -5: continue
            fusairiskDict[symbol] = fusairisk
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, fusairiskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, fusairiskDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(fusairiskDict, open(fusairiskPath, "wb"))
