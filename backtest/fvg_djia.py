...
from modules.strategy.fvg import HpFvg, GetOPSLTP
...
tf = '5 mins'
tradable = []
# df = pd.read_csv("cfd_threeBarReversal_jp.csv")
# df.columns = ["i","symbol"]
...
for symbol in symbolList:
    try:
        ...
        # # contract = Stock(symbol, 'SMART', 'USD')
        # ib.qualifyContracts(contract)
        df = ibc.GetDataDf(contractDJIA, tf)
        df2 = ibc.GetDataDf(contractMES, tf)
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = HpFvg(npArr, tf)
        npArr = oriNpArr2[:i+2]
