...
def calc_pl_for_bars(npArr, RR, lows, highs, closes):
    ...
    for i in range(5, len(npArr)-1):
        if (
            ...
            # npArr[i-4][3] < npArr[i-4][0] and
            # npArr[i-1][1] > npArr[i-2][1] and
            # npArr[i-1][2] > npArr[i-2][2] and

            # npArr[i-1][1] < npArr[i-2][1] and
            # npArr[i-1][2] > npArr[i-2][2] and
            # npArr[i-2][1] > npArr[i-3][3] and
            # npArr[i-2][1] - npArr[i-2][2] >
            # npArr[i-3][1] - npArr[i-3][2] and
            npArr[i][0] > npArr[i-1][1] and
            position == 0
        ):
            # sl = np.min(lows[i-4:i])
            sl = npArr[i-1][3]
            op = npArr[i,0]
            # op = npArr[i-1][1]
            # op = np.max(lows[i-4:i])
...
print(result_list)
csvPath = f"{rootPath}/data/gapOverHigh.csv"
