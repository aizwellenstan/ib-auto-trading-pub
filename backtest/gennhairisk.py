...
from modules.rironkabuka import GetRironFinancial, GetGennhairisk
...
def HandleGetGennhairisk(symbol):
    gennhairisk = GetGennhairisk(symbol)
    return [symbol, gennhairisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, gennhairiskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetGennhairisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            gennhairisk = result[1]
            if gennhairisk <= -6: continue
            gennhairiskDict[symbol] = gennhairisk
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, gennhairiskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, gennhairiskDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(gennhairiskDict, open(gennhairiskPath, "wb"))
