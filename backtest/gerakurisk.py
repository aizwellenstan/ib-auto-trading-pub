...
from modules.rironkabuka import GetRironFinancial, GetGerakurisk
...
def HandleGetGerakurisk(symbol):
    gerakurisk = GetGerakurisk(symbol)
    return [symbol, gerakurisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, gerakuriskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetGerakurisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            gerakurisk = result[1]
            if gerakurisk <= -6: continue
            gerakuriskDict[symbol] = gerakurisk
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, gerakuriskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, gerakuriskDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(gerakuriskDict, open(gerakuriskPath, "wb"))
