...
from modules.rironkabuka import GetRironFinancial, GetGyoushu
...
def HandleGetGyoushu(symbol):
    gyoushu = GetGyoushu(symbol)
    return [symbol, gyoushu]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, gyoushuDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetGyoushu, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, gyoushuDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, gyoushuDict, zandakaDict = UpdateData(symbolListJP)
