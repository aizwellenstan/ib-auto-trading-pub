...
from modules.rironkabuka import GetRironFinancial, GetHaitoukakutsuke
...
def HandleGetHaitoukakutsuke(symbol):
    haitoukakutsuke = GetHaitoukakutsuke(symbol)
    return [symbol, haitoukakutsuke]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, haitoukakutsukeDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetHaitoukakutsuke, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            haitoukakutsuke = result[1]
            if len(haitoukakutsuke) < 2: continue
            haitoukakutsukeDict[symbol] = haitoukakutsuke
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, haitoukakutsukeDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, haitoukakutsukeDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(haitoukakutsukeDict, open(haitoukakutsukePath, "wb"))
