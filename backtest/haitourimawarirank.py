...
from modules.rironkabuka import GetRironFinancial, GetHaitourimawarirank
...
def HandleGetHaitourimawarirank(symbol):
    haitourimawarirank = GetHaitourimawarirank(symbol)
    return [symbol, haitourimawarirank]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, haitourimawarirankDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetHaitourimawarirank, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            haitourimawarirank = result[1]
            if haitourimawarirank < 0: continue
            haitourimawarirankDict[symbol] = haitourimawarirank
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, haitourimawarirankDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, haitourimawarirankDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(haitourimawarirankDict, open(haitourimawarirankPath, "wb"))
