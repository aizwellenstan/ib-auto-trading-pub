...
from modules.rironkabuka import GetRironFinancial, GetHaitouseikourisk
...
def HandleGetHaitouseikourisk(symbol):
    haitouseikourisk = GetHaitouseikourisk(symbol)
    return [symbol, haitouseikourisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, haitouseikouriskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetHaitouseikourisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            haitouseikourisk = result[1]
            if haitouseikourisk < -5: continue
            haitouseikouriskDict[symbol] = haitouseikourisk
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, haitouseikouriskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, haitouseikouriskDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(haitouseikouriskDict, open(haitouseikouriskPath, "wb"))
