...
from modules.financials import GetInterestExpenseOperatingIncome
...
def HandleGetInterestExpenseOperatingIncome(symbol):
    interestExpense, operatingIncome = GetInterestExpenseOperatingIncome(symbol)
    return [symbol, interestExpense, operatingIncome]
...
def UpdateData(symbolList):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetInterestExpenseOperatingIncome, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            interestExpense = result[1]
            operatingIncome = result[2]
            if len(interestExpense) > 0:
                interestExpenseDict[symbol] = interestExpense
            if len(operatingIncome) > 0:
                operatingIncomeDict[symbol] = operatingIncome
    return interestExpenseDict, operatingIncomeDict
...
def main(update=False):
    ...
    if update:
        interestExpenseDict, operatingIncomeDict = UpdateData(symbolListJP)
        ...
        pickle.dump(interestExpenseDict, open(interestExpensePath, "wb"))
        pickle.dump(operatingIncomeDict, open(operatingIncomePath, "wb"))
