...
from modules.kabuyoho import GetKabukarange
...
def HandleGetKabukarange(symbol):
    kabukarange = GetKabukarange(symbol)
    return [symbol, kabukarange]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, kabukarangeDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetKabukarange, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            kabukarange = result[1]
            if len(kabukarange) < 1: continue
            kabukarangeDict[symbol] = kabukarange
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, kabukarangeDict, zandakaDict
...
def main(update=False):
    ...
    kabukarangePath = f"{rootPath}/backtest/pickle/pro/compressed/kabukarange.p"
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, kabukarangeDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(kabukarangeDict, open(kabukarangePath, "wb"))
