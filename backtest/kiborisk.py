...
from modules.rironkabuka import GetRironFinancial, GetKiborisk
...
def HandleGetKiborisk(symbol):
    kiborisk = GetKiborisk(symbol)
    return [symbol, kiborisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, kiboriskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetKiborisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, kiboriskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, kiboriskDict, zandakaDict = UpdateData(symbolListJP)
