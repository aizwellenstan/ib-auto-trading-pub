...
def checkForOpen():
    ...
    for s in assetDebtList:
        ...
        price_book_ratio = s['price_book_ratio']
        price_book_fq = s['price_book_fq']
        price_earnings_ttm = s['price_earnings_ttm']
        price_free_cash_flow_ttm = s['price_free_cash_flow_ttm']
        price_revenue_ttm = s['price_revenue_ttm']
        close = s['close']
        vwap = s['vwap']
        sales = marketcap/price_sales_ratio 
        total_asset_turnover = sales/total_assets
        book = marketcap/price_book_ratio
        book_fq = marketcap/price_book_fq
        earnings = marketcap/price_earnings_ttm
        free_cash_flow = marketcap/price_free_cash_flow_ttm
        revenue = marketcap/price_revenue_ttm
        cash_revenue = free_cash_flow/revenue
        cash_earnings = free_cash_flow/earnings
        assetDebtValList.append(cash_earnings)

    assetDebtValList.sort(reverse=True)
