...
import sys
sys.path.append('../../')
from modules.aiztradingview import GetProfit,GetProfitWithADR,GetScannerWithAttribute,GetDRREITMore,GetAssetDebt
import pickle
import gc
...
def checkForOpen():
    ...
    for trade in trades:
        ...
        if op == None: continue
        if vol < 0: continue
        if sym not in filterSymList and sym not in DRREITSymList: continue
        # print(sym)
        if op < 7.74: continue
        ...
        for s in assetDebtList:
            if s['s'] == sym:
                ...
                break
        print(sym,total_current_assets,total_liabilities_fy)
    ...
    # if total > maxProfit:
    #     maxProfit = total
    #     maxPreTrange = preTrange
    # print("maxProfit", maxProfit)
    # print("maxPreTrange", maxPreTrange)
    # preTrange += 0.01
