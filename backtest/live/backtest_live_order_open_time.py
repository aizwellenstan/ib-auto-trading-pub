...
def checkForOpen():
    ...
    while oredrOpenMinutes <= 390:
        ...
        for trade in trades:
            ...
            tp = op + adrRange * 4.36 #op + (op-sl) * tpVal
            ...
            volMax = int(total_cash/3/(op*1.003032140691))
            ...
            orderOpenTime = marketOpenTime + timedelta(minutes=oredrOpenMinutes)

            trade['status'] = ''
            
            for i in dataM1:
                if i.date >= cancelTime: continue
                if i.high >= op and i.date>orderOpenTime:
                    if i.low>op: op = i.open
                    trade['status'] = i.date
                    break
        ...
        if total > maxProfit:
            maxProfit = total
            maxOrderOpenMinutes = oredrOpenMinutes
        print("maxProfit", maxProfit)
        print("maxOrderOpenMinutes", maxOrderOpenMinutes)
        oredrOpenMinutes += 1
