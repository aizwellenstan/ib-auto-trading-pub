...
import sys
sys.path.append('../../')
from modules.aiztradingview import GetProfit, GetProfitWithADR, GetScannerWithAttribute
import pickle
import gc
...
def checkForOpen():
    ...
    while idx < len(attributeValList):
        ...
        for trade in trades:
            ...
            sl = normalizeFloat(op - adrRange * 0.03, 0.01)
            # sl = normalizeFloat(op - adrRange * 0.06581205959, 0.01)
            if op - sl < 0.01: continue

            tpVal = 20 #2.42
            tp = op + adrRange * 1.58 #op + (op-sl) * tpVal
            ...
            cancelTime = marketOpenTime+timedelta(minutes=389)
