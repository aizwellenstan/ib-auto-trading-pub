...
def checkForOpen():
    ...
    while tpVal <= 100:
        ...
        for trade in trades:
            ...
            tp = op + adrRange * tpVal #op + (op-sl) * tpVal
            
            if preMaxHigh - preMinLow > 0.58:
                tp = op + (preMaxHigh - preMinLow) * 1.14516129032

            if (
                hisBarsD1[-1+shift].open > hisBarsD1[-6+shift].low and
                hisBarsD1[-1+shift].open < hisBarsD1[-6+shift].high
            ):
                tp = op + adrRange * 0.47785276771
            ...
            gap = hisBarsD1[-1+shift].open/hisBarsD1[-2+shift].close
            if gap > 1.035416666666666:
                tp = op + adrRange * 1.32285936634
            if adrRange > 2:
                tp = op + adrRange * 1.25
            ...
            cancelTime = marketOpenTime+timedelta(minutes=38)
            exitTime = marketOpenTime+timedelta(minutes=155)
