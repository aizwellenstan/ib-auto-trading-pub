...
import sys
sys.path.append('../../')
from modules.aiztradingview import GetProfit,GetProfitWithADR,GetScannerWithAttribute,GetDRMore,GetREITMore,GetAssetDebt,GetYearPerformance
import pickle
import gc
...
def checkForOpen():
    ...
    for trade in trades:
        ...
        if sym in filterSymList:
            if sym not in perfList: continue
        if op < 7.74: continue
