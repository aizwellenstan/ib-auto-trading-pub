...
# @njit
def checkOP(npArr, i, bar, trades):
    longOP = 0
    longSL = 0
    longTP = 0
    shortOP = 0
    ...
    lowSigArr12 = np.empty(0)

    for j in range(1, bar+1):
        lowSigArr1 = np.append(lowSigArr1,npArr[i-j][2])
        lowSigArr2 = np.append(lowSigArr2,npArr[i-j-bar][2])
        lowSigArr3 = np.append(lowSigArr3,npArr[i-j-bar*2][2])
        lowSigArr4 = np.append(lowSigArr4,npArr[i-j-bar*3][2])
        lowSigArr5 = np.append(lowSigArr5,npArr[i-j-bar*4][2])
        lowSigArr6 = np.append(lowSigArr6,npArr[i-j-bar*5][2])
        lowSigArr7 = np.append(lowSigArr7,npArr[i-j-bar*6][2])
        lowSigArr8 = np.append(lowSigArr8,npArr[i-j-bar*7][2])
        lowSigArr9 = np.append(lowSigArr9,npArr[i-j-bar*8][2])
        lowSigArr10 = np.append(lowSigArr10,npArr[i-j-bar*9][2])
        lowSigArr11 = np.append(lowSigArr11,npArr[i-j-bar*10][2])
        lowSigArr12 = np.append(lowSigArr12,npArr[i-j-bar*11][2])
        highSigArr1 = np.append(highSigArr1,npArr[i-j][1])
        highSigArr2 = np.append(highSigArr2,npArr[i-j-bar][1])
        highSigArr3 = np.append(highSigArr3,npArr[i-j-bar*2][1])
        highSigArr4 = np.append(highSigArr4,npArr[i-j-bar*3][1])
        highSigArr5 = np.append(highSigArr5,npArr[i-j-bar*4][1])
        highSigArr6 = np.append(highSigArr6,npArr[i-j-bar*5][1])
        highSigArr7 = np.append(highSigArr7,npArr[i-j-bar*6][1])
        highSigArr8 = np.append(highSigArr8,npArr[i-j-bar*7][1])
        highSigArr9 = np.append(highSigArr9,npArr[i-j-bar*8][1])
        highSigArr10 = np.append(highSigArr10,npArr[i-j-bar*9][1])
        highSigArr11 = np.append(highSigArr11,npArr[i-j-bar*10][1])
        highSigArr12 = np.append(highSigArr12,npArr[i-j-bar*11][1])
    ...
    low12 = lowSigArr12.min()

    if (
        npArr[i-bar*11+1][3] < npArr[i-bar*12][0] and
        high11 < high12 and
        low11 > low12 and
        high10 < high12 and
        low10 > low12 and
...
def backtest(npArr, bar):
    ...
    for i in range(2, len(npArr)):
        ...
        trades,longOP,longSL,longTP,shortOP,shortSL,shortTP = checkOP(npArr, i, bar,trades)
        if longOP > 0:
...
def GetBestMA(symbol):
    ...
    while hour <= 4 or hour >= 22:
        ...
        bar = 1
        while bar < 31:
            totalGain, totalLoss, trades, wr = backtest(npArr, bar)
            ...
            if rr > 1:
                ...
                if trades > 0 and capital > 2:
                    ...
                    #     print(capital)
            bar += 1
    print(f"maxCapital {maxCapital} maxRR {maxRR} maxProfitPerTrade {maxProfitPerTrade} bestTpVal {bestTpVal} bestTime {bestHour}:{bestMinute} minRiskOfRuin {minRiskOfRuin} bestBar {bestBar}")
    return 0
