...
def cleanData(df):
    ...
    df = df[['open','high','low','close','eod','sod','emaX','emaY','ema500','highBias500','lowBias500','bias']]
    # df.to_csv(csvPath)
    npArr = df.to_numpy()
    return npArr
...
def backtest(npArr):
    ...
    for i in range(2, len(npArr)):
        ...
        emaY = npArr[i-1][7]
        ema500 = npArr[i-1][8]
        highBias500 = npArr[i-1][9]
        lowBias500 = npArr[i-1][10]
        bias = npArr[i-1][11]
...
def GetBestMA(symbol):
    ...
    df['ema500'] = ema500
    df['highBias500'] = (df['high']-ema500)/ema500
    df['lowBias500'] = (ema500-df['low'])/ema500
    df['bias'] = abs(df['close']-ema500)/ema500
    x = 2
    while x < 48:
        y = x+1
        if x not in emaDict:
