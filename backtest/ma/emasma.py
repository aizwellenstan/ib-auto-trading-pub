...
def cleanData(df):
    ...
    df = df.iloc[idx]
    df = df[['open', 'close', 'smaX', 'emaX', 'eod', 'sod']]
    npArr = df.to_numpy()
    return npArr
...
def backtest(npArr):
    ...
    for i in range(2, len(npArr)):
        if npArr[i][4] > 0:
            ...
            continue
        if (
            npArr[i-2][1] < npArr[i-2][2] and
            npArr[i-1][1] > npArr[i-1][2] and
            npArr[i-2][1] < npArr[i-2][3] and
...
def main():
    ...
    df = df.assign(sod=np.where(((df['hour']==22) & (df['minute']==33)), 1, 0))
    ...
    while x < 57:
        ...
        while y < 57:
            ...
            if capital > maxCapital:
                ...
                bestX = x
                bestY = y
                print(f"maxCapital {maxCapital} bestSma {bestX} bestEma {bestY}")
            y += 1
        x += 1
    print(capital)
    print(f"maxCapital {maxCapital} bestSma {bestX} bestEma {bestY}")
