...
from modules.yf import GetData
...
def cleanData(df):
    ...
    df = df[['open','high','low','close','volume','eod']]
...
# @njit
def checkOP(npArr, i, bar, trades, totalVol):
    longOP = 0
    longSL = 0
    longTP = 0
    shortOP = 0
    shortSL = 0
    shortTP = 0
    avgVolume = totalVol/20
    if (
        # # Down Thrust
        # (
        #     npArr[i-1][4] > npArr[i-2][4] and
        #     npArr[i-1][3] - npArr[i-1][2] >
        #     (npArr[i-1][1] - npArr[i-1][2]) * 0.05 and
        ...
        # # Selling Climax
        # (
        #     npArr[i-1][4] > avgVolume and
        #     npArr[i-1][3] < npArr[i-1][0] and
        #     npArr[i-1][3] - npArr[i-1][2] >
        #     (npArr[i-1][1] - npArr[i-1][2]) * 0.25
        # ) 
        # or
        # # Bullish Effort Lower Than Bullish Result
        # (
        #     npArr[i-1][4] < npArr[i-2][4] and
        #     npArr[i-2][3] < npArr[i-2][0] and
        #     npArr[i-1][3] < npArr[i-1][0] and
        #     npArr[i-1][0]-npArr[i-1][3] >
        #     npArr[i-2][0]-npArr[i-2][3] 
        # ) 
        # or
        # # Bullish Effort Bigger Than Bullish Result
        # (
        #     npArr[i-1][4] > npArr[i-2][4] and
        #     npArr[i-2][3] < npArr[i-2][0] and
        #     npArr[i-1][3] < npArr[i-1][0] and
        #     npArr[i-1][0]-npArr[i-1][3] <
        #     npArr[i-2][0]-npArr[i-2][3] and
        #     npArr[i-1][0]-npArr[i-1][3] <
        #     abs(npArr[i-4][0]-npArr[i-4][3])
        # ) 
        # or
        # # Inverse Down Thrust
        # (
        ...
        #     abs(npArr[i-2][3]-npArr[i-2][0])>
        #     abs(npArr[i-3][3]-npArr[i-3][0])and
        #     npArr[i-1][3] > npArr[i-1][0]
        # ) 
        # or
        # # Pseudo Down Thrust
        # (
        #     # npArr[i-1][4] < npArr[i-3][4] and
        #     # npArr[i-1][4] < npArr[i-2][4] and
        #     (npArr[i-1][3] - npArr[i-1][2]) /
        #     (npArr[i-1][1] - npArr[i-1][2]) > 0.2
        # ) 
        # or
        # No Supply Bar
        (
        ...
        # or
        # Pseudo Inverse Down Thrust
        # (
        #     npArr[i-1][4] < npArr[i-3][4] and
        #     npArr[i-1][4] < npArr[i-2][4] and
        #     abs(npArr[i-1][0]-npArr[i-1][3]) <
        #     abs(npArr[i-2][0]-npArr[i-2][3]) and
        #     npArr[i-1][1] - npArr[i-1][3] >
        #     npArr[i-1][3] - npArr[i-1][2]
        # )
    ):
        ...
        # ) 
        # or
        # # Bearish Effort Lower Than Bearish Result
        # (
        #     npArr[i-1][4] < npArr[i-2][4] and
        #     npArr[i-2][3] > npArr[i-2][0] and
        #     npArr[i-1][3] > npArr[i-1][0] and
        ...
        # ) 
        # or
        # # Bearish Effort Bigger Than Bearish Result
        # (
        #     npArr[i-1][4] > npArr[i-2][4]*1.9 and
        #     npArr[i-2][3] > npArr[i-2][0] and
        #     npArr[i-1][3] > npArr[i-1][0] and
        #     npArr[i-1][3]-npArr[i-1][0] <
        #     npArr[i-2][3]-npArr[i-2][0] and
        #     npArr[i-1][3]-npArr[i-1][0] <
        #     abs(npArr[i-4][3]-npArr[i-4][0])
        # ) 
        # or
        # # Inverse Up Thrust
        # (
        ...
        # ) 
        # or
        # # Failed Effort Buying Climax
        # (
        #     npArr[i-2][4] > npArr[i-3][4] and
        #     abs(npArr[i-2][3]-npArr[i-2][0])/(npArr[i-2][1]-npArr[i-2][2]) >
        #     abs(npArr[i-3][3]-npArr[i-3][0])/(npArr[i-3][1]-npArr[i-3][2]) and
        #     npArr[i-1][3] < npArr[i-1][0]
        # ) 
        # or
        # # Pseudo Up Thrust
...
def backtest(npArr, bar, volumeList):
    totalGain = 0
    totalLoss = 0
    win = 0
    loss = 0
    ...
    for i in range(2, len(npArr)):
        ...
        #                 newShortTrades.append((op, sl, tp, condition))
        #     shortTrades = newShortTrades
        volumeList = volumeList[i-21:i-1]
        totalVol = sum(volumeList)
        trades,longOP,longSL,longTP,shortOP,shortSL,shortTP = checkOP(npArr, i, bar,trades, totalVol)
        if longOP > 0:
...
def GetBestMA(symbol):
    ...
    while hour <= 4 or hour >= 22:
        ...
        bar = 1
        while bar < 2:
            totalGain, totalLoss, trades, wr = backtest(npArr, bar, volumeList)
