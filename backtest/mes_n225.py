...
from modules.aiztradingview import GetJPETF, GetIndexHoldingsJP, GetLiquidETF
from ib_insync import *
from modules.strategy.arbitrage import Arb225ESNQ
...
FUCK_ASIA_START = time_to_minutes("22:00")
FUCK_ASIA_END = time_to_minutes("23:59")
OVERNIGHT_START = time_to_minutes("00:00")
OVERNIGHT_END = time_to_minutes("05:00")
# NY_END = time_to_minutes("23:10") #10 mins
def is_ny_op(time_str):
    time_in_minutes = time_to_minutes(time_str)
    # return (
    #     (time_in_minutes >= FUCK_ASIA_START and 
    #     time_in_minutes < FUCK_ASIA_END) or
    #     (
    #         time_in_minutes >= OVERNIGHT_START and 
    #         time_in_minutes < OVERNIGHT_END
    #     )
    # )
...
def GetSignal(npArrSig, npArrNQ, npArrMNTPX, npArr225):
    if len(npArr225) < 192: return 0
    atrPeriod = 200
    npArr = npArr225
    atr = ATR(npArr[:-1][:,1][-atrPeriod:],npArr[:-1][:,2][-atrPeriod:],npArr[:,3][:-1][-atrPeriod:])
    # atrSig = ATR(npArrSig[:-1][:,1][-atrPeriod:],npArrSig[:-1][:,2][-atrPeriod:],npArrSig[:,3][:-1][-atrPeriod:])
    if np.max(npArr[-3:][:,1]) - np.min(npArr[-3:][:,2]) > atr * 3:
        # np.max(npArrSig[-3:][:,1]) - np.min(npArrSig[-3:][:,2]) > atrSig * 3 and
        if (
            npArrSig[-2][3] > npArrSig[-2][0] and
            npArrNQ[-2][3] > npArrNQ[-2][0] and
            # npArr225[-2][3] < npArr225[-2][0] and
            (npArrSig[-2][3] - npArrSig[-2][0]) / npArrSig[-2][0] >
            abs(npArr225[-2][3] - npArr225[-2][0]) / npArr225[-2][0]
        ): 
            pe = permutation_entropy(npArr[:-1][-212:][:,3])
            sma = SmaArr(pe, 40)
            if pe[-1] < sma[-1]:
                # pe = permutation_entropy(npArrMNTPX[:-1][-212:][:,3])
                # sma = SmaArr(pe, 40)
                # if pe[-1] < sma[-1]:
                pe = permutation_entropy(npArrSig[:-1][-212:][:,3])
                sma = SmaArr(pe, 40)
                if pe[-1] > sma[-1]:
                    return 1
        elif (
            ...
            npArrNQ[-2][3] < npArrNQ[-2][0] and
            # npArr225[-2][3] > npArr225[-2][0] and
            abs(npArrSig[-2][3] - npArrSig[-2][0]) / npArrSig[-2][0] >
            (npArr225[-2][3] - npArr225[-2][0]) / npArr225[-2][0]
        ):
            pe = permutation_entropy(npArr[:-1][-212:][:,3])
            sma = SmaArr(pe, 40)
            if pe[-1] < sma[-1]:
...
contractNQ = Future(conId=637533450, symbol='NQ', lastTradeDateOrContractMonth='20240920', multiplier='20', currency='USD', localSymbol='NQU4', tradingClass='NQ')
ib.qualifyContracts(contractNQ)
...
for symbol in symbolList:
    try:
        ...
        df2 = ibc.GetDataDf(contractES, tf)
        df3 = ibc.GetDataDf(contractNQ, tf)
        df4 = ibc.GetDataDf(contractJPY, tf)
        ...
        df3 = df3[df3['date'].isin(df['date'])]
        df4 = df4[df4['date'].isin(df['date'])]
    ...
    df3['Low'] = df3['low']
    df3['Close'] = df3['close']
    df3['Volume'] = df3['volume']
    df4['Open'] = df4['open']
    df4['High'] = df4['high']
    df4['Low'] = df4['low']
    df4['Close'] = df4['close']
    df4['Volume'] = df4['volume']
    print(df)

    oriNpArr = df[['Open', 'High', 'Low', 'Close', 'Volume']].to_numpy()
    oriNpArr2 = dfOri[['Open', 'High', 'Low', 'Close', 'Volume']].to_numpy()
    npArrNQ = df3[['Open', 'High', 'Low', 'Close', 'Volume']].to_numpy()
    npArrJPY = df4[['Open', 'High', 'Low', 'Close', 'Volume']].to_numpy()
    print(oriNpArr)
    ...
    trades = []
    oriNpArrNQ = npArrNQ
    oriNpArrJPY = npArrJPY
    for i in range(0, len(oriNpArr)):
        npArrSig = oriNpArr[:i+2]
        npArr225 = oriNpArr2[:i+2]
        npArrNQ = oriNpArrNQ[:i+2]
        npArrJPY = oriNpArrJPY[:i+2]
        signal = 0
        # if i > 0:
        #     signal = GetSignal(npArrSig, npArrNQ, npArrMNTPX, npArr225)
        sl = 0
        op = 0
        tp = 0
        if i > 0:
            signal, op, sl, tp = Arb225ESNQ(npArr225, npArrSig, npArrNQ, npArrJPY, tf, tick_val=5)
    ...
    if (
        sharpe > 0.08 and profitFactor > 2.866 and
        sortino > 249 and returns_ann > 30
    ):
