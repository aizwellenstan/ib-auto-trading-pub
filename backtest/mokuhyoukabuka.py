...
from modules.kabuyoho import GetMokuhyoukabuka
...
def HandleGetMokuhyoukabuka(symbol):
    mokuhyoukabuka = GetMokuhyoukabuka(symbol)
    return [symbol, mokuhyoukabuka]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, mokuhyoukabukaDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetMokuhyoukabuka, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            mokuhyoukabuka = result[1]
            if mokuhyoukabuka < 1: continue
            mokuhyoukabukaDict[symbol] = mokuhyoukabuka
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, mokuhyoukabukaDict, zandakaDict
...
def main(update=False):
    ...
    mokuhyoukabukaPath = f"{rootPath}/backtest/pickle/pro/compressed/mokuhyoukabuka.p"
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, mokuhyoukabukaDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(mokuhyoukabukaDict, open(mokuhyoukabukaPath, "wb"))
