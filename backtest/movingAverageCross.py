...
def HandleGetDataLts(symbol):
    return[symbol, GetDataLts(symbol)]
...
def main(update=False):
    ...
    if update:
        ...
        with ThreadPoolExecutor(max_workers=100) as executor:
            futures = [executor.submit(HandleGetDataLts, symbol) for symbol in symbolListUS]
        ...
        dataDictJP = {}
        with ThreadPoolExecutor(max_workers=100) as executor:
            futures = [executor.submit(HandleGetDataLts, symbol) for symbol in symbolListJP]
            for future in as_completed(futures):
                result = future.result()
                if len(result[1]) < 3: continue
