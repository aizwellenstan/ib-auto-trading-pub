...
from modules.strategy.arbitrage import Arb225TPX
...
for symbol in symbolList:
    try:
        ...
        df2 = ibc.GetDataDf(contractMNTPX, tf)
    ...
    for i in range(0, len(oriNpArr)):
        n225Arr = oriNpArr2[:i+2]
        topixArr = oriNpArr[:i+2]
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = Arb225TPX(n225Arr, topixArr, tf, 5)
        # npArr = oriNpArr2[:i+2]
