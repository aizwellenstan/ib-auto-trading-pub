...
def main():
    ignore = ['UVIX','DDD','AMC','FSTX','BOIL','BBBY','SI','CVNA']
    currency = 'USD'
    closeDict = GetClose()
    ...
    for symbol, close in closeDict.items():
        ...
        # if CheckHaveOption(symbol):
            ...
            for optionschain in chains:
                ...
                oneStrikeAway = False
                twoStrikeAway = False
                threeStrikeAway = False
                for strike in strikeList:
                    if error: break
                    if strike < close - 0.5:
                        if not oneStrikeAway:
                            oneStrikeAway = True
                            continue
                        if not twoStrikeAway:
                            twoStrikeAway = True
                            continue
                        if not threeStrikeAway:
                            threeStrikeAway = True
                            continue
    ...
    nakedPutsPath = f'{rootPath}/data/NakedPuts3StrikeAway.csv'
