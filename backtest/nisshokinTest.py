    ...
    for i in range(idx, len(npArr)):
        ...
        if date in zandakaDict:
            ...
            if currentZandaka[0] < currentZandaka[1]:
                ...
                if urizanHiritsu > 0.00011311933532638829:
    ...
    for i in range(0, length):
        if position < 1:
            ...
            for symbol, npArr in cleanDataDict.items():
                npArr = npArr[0:i]
                res = CheckShortSqueezeOri(symbol, cleanSignalDict, ordinarySharesDict, npArr)
            ...
            op = float(zandakaSignalList[0][1])
            sl = float(zandakaSignalList[0][2])
            curZandakaHiritsu = zandakaSignalList[0][4]
            if zandakaSignalList[0][6] != "":
                tp = float(zandakaSignalList[0][5])
            else:
                tp = op + 19
            position = 1
            gain = 1
            if cleanDataDict[positionSymbol][i][2] < sl:
                ...
                position = 0
            if gain < 1:
                failed.append(curZandakaHiritsu)
        else:
            if cleanDataDict[positionSymbol][i][2] < sl:
                gain = sl/op
                balance *= gain
