...
from modules.financials import GetOrdinaryShares
...
def HandleGetFreeCashflow(symbol):
    ordinaryShares = GetOrdinaryShares(symbol)
    return [symbol, ordinaryShares]
...
def UpdateData(symbolList):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        ...
        for future in as_completed(futures):
            ...
            symbol = result[0]
            ordinaryShares = result[1]
            if len(ordinaryShares) > 0:
                ordinarySharesDict[symbol] = ordinaryShares
    return ordinarySharesDict
...
def main(update=False):
    ...
    if update:
        ordinarySharesDict = UpdateData(symbolListJP)
        ...
        pickle.dump(ordinarySharesDict, open(ordinarySharesPath, "wb"))
