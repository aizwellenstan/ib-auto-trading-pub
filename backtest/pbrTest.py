...
def calculate_balance(floatSharesLimit):
    balance = 1
    for i in range(2, length):
        topPbr = 1
        topSymbol = ""
        pbr_heap = []
        for symbol, npArr in dataDict.items():
            if symbol not in floatShareDict or symbol not in assetDict:
                continue
            if floatShareDict[symbol] > floatSharesLimit:
                continue
            pbr = npArr[i - 1][3] * floatShareDict[symbol] / assetDict[symbol]
            heapq.heappush(pbr_heap, (pbr, symbol))
        
        if not pbr_heap:
            continue
        topPbr, topSymbol = heapq.heappop(pbr_heap)
        npArr = dataDict[topSymbol]
        gain = npArr[i][3] / npArr[i][0]
        balance *= gain
...
num_cores = Parallel(n_jobs=-1)
results = num_cores(delayed(calculate_balance)(floatSharesLimit) for floatSharesLimit in floatSharesList)

maxBalance = max(results)
maxFloatSharesLimit = floatSharesList[results.index(maxBalance)]
