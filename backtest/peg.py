...
from modules.irbank import GetZandaka, GetDividend, GetPeg
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, pegDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetPeg, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            peg = result[1]
            if len(peg) < 2: continue
            pegDict[symbol] = peg
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, pegDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, pegDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(pegDict, open(pegPath, "wb"))
