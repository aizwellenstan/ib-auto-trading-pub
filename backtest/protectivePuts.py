...
def GetPut(chains, price):
    for optionschain in chains:
        ...
        for strike in strikeList:
            if strike >= price:
                optionContract = ibc.GetOptionPutContract(symbol, expir, strike)
...
for symbol, data in divPerDict.items():
    ...
    chains = ibc.GetChains(symbol)
    bid, ask, spread, expir, strike = GetPut(chains, close)
    if bid <= 0: continue
    print(symbol, bid, expir, strike)
    option_cost = (ask * multiplier * 100 + 4)
    if profit - option_cost > 4.25:
        profit -= ((spread * 2) * 100 + 4)
