...
from modules.sbi import GetPts
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, ptsDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetPts, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, ptsDict, zandakaDict
...
def main(update=False):
    ...
    ptsPath = f"{rootPath}/backtest/pickle/pro/compressed/pts.p"
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, ptsDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(ptsDict, open(ptsPath, "wb"))
