...
from modules.aiztradingview import GetProfit,GetFloat,GetPE,GetADR,GetIndustry,GetSector,GetVol,GetMarketCap, GetNetDebt, GetEPS
import requests
from scipy.signal import lfilter
from modules.riskOfRuin import calcRisk
from collections import defaultdict
from modules.trend import GetTrend
from modules.shareholders import GetInsiderPercent, GetInstitutions
...
df = pd.read_csv (r'./csv/trades_8M.csv', index_col=0)
df.drop
...
def main():
    try:
        ...
        adrDict = GetADR('USD')
        floatDict = GetFloat('USD')
        industryDict = GetIndustry('USD')
        sectorDict = GetSector()
        volDict = GetVol('USD')
        marketCapDict = GetEPS('USD')

        volList = []
        for key in volDict:
            volList.append(volDict[key])
        volList.sort()

        # marketCapList = []
        # for key in marketCapDict:
        #     if marketCapDict[key] < 0: continue
        #     marketCapList.append(marketCapDict[key])
        # marketCapList.sort()
        ...
        for trade in trades:
            ...
            institutions = GetInstitutions(symbol)
            # insiderPercentDict[symbol] = insiderPercent
            institutionsDict[symbol] = GetInstitutions(symbol)

        institutionsList = []
        for key in institutionsDict:
            institutionsList.append(institutionsDict[key])
        institutionsList.sort(reverse=True)
        ...
        oldVolLimit = 0
        idx = 0
        while idx < len(institutionsList):
            total = 0
            net = 0
            win = 0
            loss = 0
            totalNetProfit = 0
            totalNetLoss = 0
            # volLimit = volList[idx]
            # if volLimit < oldVolLimit + 100: 
            #     idx += 1
            #     continue
            # oldVolLimit = volLimit
            for trade in trades:
                industryLeaderBoard = {}
                industryAvgBoard = {}
                sectorLeaderBoard = {}
                ...
                # if hisBarsD1[0].open < hisBarsD1[1].close * 1.005: 
                #     continue
                ...
                if symbol in shortableSymList:
                    ...
                    val = institutionsDict[symbol]
                    if val < institutionsList[idx]: continue
                ...
                # if hisBarsD1[1].close < hisBarsD1[3].open * adjVal: 
                #     continue
                ...
                # if (
                #     hisBarsD1[4].close / hisBarsD1[4].open < 0.9858333333333334 and
                #     hisBarsD1[3].close / hisBarsD1[3].open < 0.8967254408060454 and
                #     hisBarsD1[1].close / hisBarsD1[1].open < 0.9370629370629373
                # ): continue

                # if  hisBarsD1[1].close / hisBarsD1[1].open > 0.926610644257703:
                # if hisBarsD1[1].close / hisBarsD1[3].open > 1.28: continue
                ...
                #                 if sym2 not in volDict: continue
                #                 if sym2 not in marketCapDict: continue
                #                 sym2Vol = volDict[sym2]
                #                 sym2MarketCap = marketCapDict[sym2]
                #                 sym2MarketCap = marketCapDict[symbol]
                #                 # if sym2Vol < volList[idx]: continue
                #                 if sym2MarketCap < marketCapList[idx]: continue
                #                 # if sym2MarketCap < symMarketCap: continue
                ...
                #                 industryAvgArr.append(industryGain)
                #         industryLeaderBoard[industry] = industryLeader
                #         if len(industryAvgArr) > 0:
                #             industryAvg = sum(industryAvgArr)/len(industryAvgArr)
                #         industryAvgBoard[industry] = industryAvg
                # else:
                #     industryCheck = True
                            
                # if (
                #     curIndustryGain < industryLeader * 0.99 and
                #     curIndustryGain < industryAvg * 1.18
                # ):  industryCheck = True
                ...
                #         sectorAvg = sectorAvgBoard[sector]
                #     else:
                #         sectorAvgArr = []
                #         for sym2 in groupList:
                #             if sym2 == symbol: continue
                ...
                #                 sectorAvgArr.append(sectorGain)
                #         sectorLeaderBoard[sector] = sectorLeader
                #         sectorAvg = sum(sectorAvgArr)/len(sectorAvgArr)
                #         sectorAvgBoard[sector] = sectorAvg
                # else:
                #     sectorCheck = True
                ...
                #     avgH2L1 = df.loc[bearCandle, 'h1l1'].mean()

                #     H2L1 = hisBarsD1[1].high / hisBarsD1[1].low
                #     if (
                #         H2L1 > avgH2L1
                #     ): continue
                ...
                testhisBarsH2 = list(filter(lambda x:x.date >= backtestTime,dataH2))
                trade['status'] = ''
                
                cancelTime = backtestTime+timedelta(minutes=38)
                for i in testhisBarsH2:
                    if i.high >= op:
            ...
            if(total > maxProfit + 1): # and winrate>0.067
                ...
                maxProfit = total
                maxTpVal = tpVal
                maxAdjVal = '{0:.10f}'.format(institutionsList[idx])
