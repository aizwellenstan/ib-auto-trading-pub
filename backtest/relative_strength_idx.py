...
from modules.aiztradingview import GetProfit, GetProfitJP, GetVolFloat, GetROA, GetROI, GetFloat,GetPE,GetADR,GetIndustry,GetSector,GetVol,GetMarketCap
import requests
...
def main():
    ...
    try:
        ...
        volFloatDict = GetVolFloat(currency)
        ...
        volFloatList = []
        for key in volFloatDict:
            volFloatList.append(volFloatDict[key])
        volFloatList.sort()
        ...
        while idx < len(volFloatList):
            ...
            for trade in trades:
                ...
                if symbol in shortableSymList:
                    if symbol not in volFloatDict: continue
                    volFloat = volFloatDict[symbol]
                    if volFloat < volFloatList[idx]: continue
                # if currency == 'USD':
                #     if symbol not in shortableSymList: continue
                ...
                # if hisBarsD1[0].open < hisBarsD1[1].close * 1.006: 
                #     continue
                ...
                # tp = hisBarsD1[1].close + adrRange * adjVal
                # tp = NormalizeFloat(tp, basicPoint)
                ...
                #     ): continue
                    ...
                    # if (
                    #     hisBarsQQQD1[3].close > hisBarsQQQD1[3].open and
                    #     hisBarsQQQD1[2].close > hisBarsQQQD1[2].open and
                    #     hisBarsQQQD1[1].close > hisBarsQQQD1[1].open and
                    #     hisBarsSPYD1[3].close > hisBarsSPYD1[3].open and
                    ...
                    #     # hisBarsIWMD1[2].close > hisBarsIWMD1[2].open and
                    #     # hisBarsIWMD1[1].close > hisBarsIWMD1[1].open and
                    #     hisBarsD1[3].close < hisBarsD1[3].open * adjVal and
                    #     hisBarsD1[2].close < hisBarsD1[2].open * adjVal and
                    #     hisBarsD1[1].close < hisBarsD1[1].open * adjVal
                    # ): continue
                ...
                #         hisBarsD1closeArr.append(d.close)
                    ...
                    # SmaD1 = Sma(hisBarsD1avgPriceArr[1:16], 15)
                    # tp = SmaD1 + adrRange * adjVal
                    # tp = NormalizeFloat(tp, basicPoint)
                ...
                # if symbol in shortableSymList and len(hisBarsD1)>181:
                #     if hisBarsD1[1].close / hisBarsD1[150].close < adjVal:
                #         continue
                ...
                #     bias = (hisBarsD1[1].close-SmaD1)/SmaD1
                #     if bias < adjVal: continue
                #     if hisBarsD1[1].close / hisBarsD1[2].close < adjVal: continue
                ...
                #         #     curIndustryGain > industryLeader * 0.7
                #         # ):  industryCheck = True
                #         industryCheck = True
