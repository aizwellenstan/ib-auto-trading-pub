...
from modules.aiztradingview import GetProfit,GetFloat,GetPE,GetADR,GetIndustry,GetSector,GetVol,GetMarketCap,GetIncome
import requests
...
df = pd.read_csv (r'./csv/trades_7M.csv', index_col=0)
df.drop
...
def main():
    try:
        ...
        hisBarsD1Dict = {}
        output = open("./pickle/pro/compressed/hisBarsStocks7MD1Dict.p", "rb")
        gc.disable()
        ...
        hisBarsH1Dict = {}
        output = open("./pickle/pro/compressed/hisBarsStocks7MH1Dict.p", "rb")
        gc.disable()
        ...
        for key in incomeDict:
            if key in marketCapDict:
                if  marketCapDict[key] > 0:
                    incomeMarketCapList.append(incomeDict[key]/marketCapDict[key])
        incomeMarketCapList.sort()
        ...
        idx = 0
        while idx < len(incomeMarketCapList):
            total = 0
            net = 0
            win = 0
            loss = 0
            ...
            for trade in trades:
                ...
                # curVol = volDict[symbol]
                adrRange = adrDict[symbol]
                # curFloat = floatDict[symbol]
                # if curFloat < 1: continue
                # if adrRange/curFloat < incomeMarketCapList[idx]: continue

                income = incomeDict[symbol]
                marketCap = marketCapDict[symbol]
                if income/marketCap < incomeMarketCapList[idx]: continue
            ...
            if(total > maxProfit + 1): # and winrate>0.067
                ...
                maxProfit = total
                maxTpVal = tpVal
                maxAdjVal = '{0:.10f}'.format(incomeMarketCapList[idx])
