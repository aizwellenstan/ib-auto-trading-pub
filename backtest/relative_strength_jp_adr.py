...
# df = pd.read_csv (r'./csv/trades_6M_JP.csv', index_col=0)
df = pd.read_csv (r'./csv/result/trades_status_JP.csv', index_col=0)
df.drop
...
def main():
    try:
        ...
        output = open("./pickle/pro/compressed/jp/hisBarsStocks6MD1arr.p", "rb")
        gc.disable()
        ...
        hisBarsStocksH1arr = []
        output = open("./pickle/pro/compressed/jp/hisBarsStocks6MH1arr.p", "rb")
        gc.disable()
        ...
        adrValList = []
        for s in adrList:
            adrValList.append(s['adr'])
        adrValList.sort(reverse=True)
        ...
        for hisBarsStockD1 in hisBarsStocksD1arr:
            dataD1Dict[hisBarsStockD1['s']] = hisBarsStockD1['d']
        ...
        while idx <= len(adrValList):
            ...
            for trade in trades:
                ...
                if adrRange < adrValList[idx]: continue
                ...
                tp = op + adrRange * 1.97 #1.58
                # tp = op * 1.02 + adrRange
                tp = normalizeFloat(tp, 0.01, 0.01)

                # vol = trade['vol']
                if op - sl < 0.01: sl = op - 0.1
                vol = int((1000*112.2)/(op-sl))
