...
from modules.sharpe import GetSharpe, GetStd
...
def main():
    ...
    try:
        ...
        if currency == "USD":
            symbolArr = ["QQQ","SQQQ","SPY","IAU","IWM","VTI","DIA"]
        ...
        for trade in trades:
            ...
            if (
                ...
                if (
                    df.iloc[-1].Volume >
                    df.iloc[-2].Volume * (1-volVal) and
                    df.iloc[-1].Volume >
                    df.iloc[-3].Volume * (1-volVal) and
                    df.iloc[-1].Volume <
                    df.iloc[-2].Volume * (1+volVal) and
                    df.iloc[-1].Volume <
                    df.iloc[-3].Volume * (1+volVal)
                ): continue
                ...
                df = df[['Vwap']]
                sharpe = GetSharpe(df)
                if sharpe < 0.051: continue

                df = symDf
                df = df.assign(meanCol=df.Close.pct_change().rolling(252).mean())
                df = df.assign(stdCol=df.Close.pct_change().rolling(252).std())
                df = df.assign(sharpe=df.meanCol/df.stdCol)
                df = df.assign(srsma=df.sharpe.rolling(17).mean())
                df = df.assign(srsmadiff=df.srsma.diff())

                if (
                    df.iloc[-2].srsmadiff > 0 and
                    df.iloc[-1].srsmadiff < 0
                ): continue

                df = df.assign(srsma=df.sharpe.rolling(27).mean())
                df = df.assign(srsmadiff=df.srsma.diff())
                ...
                df = symDf
                df = df.assign(meanCol=df.Volume.pct_change().rolling(252).mean())
                df = df.assign(stdCol=df.Volume.pct_change().rolling(252).std())
                df = df.assign(sharpe=df.meanCol/df.stdCol)
                df = df.assign(srsma=df.sharpe.rolling(79).mean())
                df = df.assign(srsmadiff=df.srsma.diff())
                ...
                if (
                    df.iloc[-1].Volume <
                    df.iloc[-2].Volume and
                    df.iloc[-2].Volume <
                    df.iloc[-3].Volume and
                    df.iloc[-3].Volume <
                    df.iloc[-4].Volume and
                    df.iloc[-4].Volume <
                    df.iloc[-5].Volume
                ): continue

                # closeArr = df['Vwap'].values.tolist()
                
                # if len(closeArr) > 50:
                #     SmaD1 = Sma(closeArr[-50:], 50)
                #     bias = (df.iloc[-1].Vwap-SmaD1)/SmaD1
                #     if bias > 0.11: continue
                ...
                if (
                    ...
                    df = df.assign(hlRange=(df.High-df.Low)/(df.H2-df.L2)+0.01)
                    avgHLRange = df.loc[bearCandle, 'hlRange'].mean()

                    hlRange = (
                        (df.iloc[-1].High-df.iloc[-1].Low) /
                        (df.iloc[-2].High-df.iloc[-2].Low+0.01)
                    )

                    if hlRange > avgHLRange * 1.45: continue

                if len(hisBarsD1) > 26:
                    if hisBarsD1[1].close / hisBarsD1[25].close < 0.65:
                        continue
