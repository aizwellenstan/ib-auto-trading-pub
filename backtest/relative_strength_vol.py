...
from modules.aiztradingview import GetProfit,GetPE,GetADR,GetVol,GetIndustry,GetSector
import requests
...
def main():
    try:
        ...
        oldLimit = 100
        idx = 0
        while idx <= len(volValList):
            volVal = volValList[idx]
            if volVal < oldLimit + 100: 
                idx += 1
                continue
            else: oldLimit = volVal
            print(volVal)
            total = 0
            net = 0
            win = 0
            ...
            for trade in trades:
                ...
                volume90d = volDict[symbol]
                if volume90d < volVal: continue
