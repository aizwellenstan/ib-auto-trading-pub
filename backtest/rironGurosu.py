...
from modules.aiztradingview import GetCloseJP, GetClose, GetCloseAll
...
from modules.rironkabukaDividends import CheckRironkabuka
from modules.seekingalpha import GetDividend
...
portfolioPath = f'{rootPath}/data/DividendToPriceRiron.csv'
ignorePath = f'{rootPath}/data/IgnoreAll.csv'
noTradePath = f'{rootPath}/data/NoTradeRiron.csv'
resPath = f'{rootPath}/data/Gurosu.csv'
...
def CheckSharpeSortino(npArr):
    # update module every year
    # 'Sharpe Ratio','Sortino Ratio','mdd','cagr'
    if npArr[0][0] < 0.01: return False
    if npArr[0][0] < 0.57: return False
    return True
...
def GetDividendData(df, currency, seekingalpha=False):
    dividend = 0
    dividendKey = "adjusted_amount"
    if seekingalpha:
        df = GetDividend("NPNYY")
        if len(df) < 1: return 0
        df = df.assign(previousDate=df['pay_date'].shift(-1))
        df = df.assign(paymentDate = pd.to_datetime(df['pay_date']))
        df = df.assign(duration = df.paymentDate-pd.to_datetime(df.previousDate))
        df = df.set_index('paymentDate')
    else:
        try:
...
rironkabukaDict = {}
dataDict = {}
# dataDict = LoadDict(portfolioPath,'DivdendToPrice')
dump = False
...
for symbol, close in closeDictJP.items():
    ...
    if not CheckIPOPrice(symbol, currency):
        ...
        dump = True
        continue
    dataDict[symbol] = rironkabuka/close
    dump = True
...
#         dump = True
#         continue
#     if not CheckSharpeSortino(npArr) or not CheckIPOPriceM(symbol):
#         noTradeList.append(symbol)
#         dump = True
#         continue
#     if not CheckRironkabuka(symbol): continue
