...
from modules.strategy.roc import Roc
...
def is_on_hour_or_half_hour(time_str):
    hours, minutes = map(int, time_str.split(':'))
...
for symbol in symbolList:
    try:
        ...
        # ib.qualifyContracts(contract)
        df = ibc.GetDataDf(contractN225, tf)
    ...
    def is_new_date(current_date, previous_date):
        return 1 if current_date.date() != previous_date.date() else 0
    import pandas as pd
    df['date'] = pd.to_datetime(df.index)
    # print(df['date'])
    # sys.exit()
    df['is_new_date'] = df['date'].dt.date != df['date'].shift(1).dt.date
    df['Update'] = df['is_new_date'].astype(int)  # Convert boolean to integer (0 or 1)
    # df['Update'] = df['date'].apply(is_new_date)

    oriNpArr = df[['Open', 'High', 'Low', 'Close', 'Volume', 'Update']].to_numpy()
    TotalSignal = []
    SlSignal = []
    TpSignal = []
    trades = []
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = Roc(npArr, tf, 5)
