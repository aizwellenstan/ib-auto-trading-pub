...
from modules.rironkabuka import GetRironFinancial, GetRyuudoumeyasu
...
res = GetRyuudoumeyasu('1739')
print(res)
sys.exit()

def HandleGetRyuudoumeyasu(symbol):
    ryuudoumeyasu = GetRyuudoumeyasu(symbol)
    return [symbol, ryuudoumeyasu]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, ryuudoumeyasuDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetRyuudoumeyasu, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            ryuudoumeyasu = result[1]
            if len(ryuudoumeyasu) < 2: continue
            ryuudoumeyasuDict[symbol] = ryuudoumeyasu
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, ryuudoumeyasuDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, ryuudoumeyasuDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(ryuudoumeyasuDict, open(ryuudoumeyasuPath, "wb"))
