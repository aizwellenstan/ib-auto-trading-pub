...
from modules.irbank import GetZandaka, GetDividend, GetShare
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, shareDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetShare, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            share = result[1]
            if len(share) < 2: continue
            shareDict[symbol] = share
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, shareDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, shareDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(shareDict, open(sharePath, "wb"))
