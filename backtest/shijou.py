...
from modules.rironkabuka import GetRironFinancial, GetShijou
...
def HandleGetShijou(symbol):
    shijou = GetShijou(symbol)
    return [symbol, shijou]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, shijouDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetShijou, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, shijouDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, shijouDict, zandakaDict = UpdateData(symbolListJP)
