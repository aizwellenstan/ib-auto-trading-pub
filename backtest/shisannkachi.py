...
from modules.rironkabuka import GetRironFinancial, GetShisannkachi
...
def HandleGetShisannkachi(symbol):
    shisannkachi = GetShisannkachi(symbol)
    return [symbol, shisannkachi]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, shisannkachiDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetShisannkachi, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            shisannkachi = result[1]
            if shisannkachi < 1: continue
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, shisannkachiDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, shisannkachiDict, zandakaDict = UpdateData(symbolListJP)
