...
from modules.rironkabuka import GetRironFinancial, GetShisannrisk
...
def HandleGetShisannrisk(symbol):
    shisannrisk = GetShisannrisk(symbol)
    return [symbol, shisannrisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, shisannriskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetShisannrisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            shisannrisk = result[1]
            if shisannrisk < -6: continue
            shisannriskDict[symbol] = shisannrisk
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, shisannriskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, shisannriskDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(shisannriskDict, open(shisannriskPath, "wb"))
