...
from modules.rironkabuka import GetRironFinancial, GetShuueki
...
def HandleGetShuueki(symbol):
    shuueki = GetShuueki(symbol)
    return [symbol, shuueki]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, shuuekiDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetShuueki, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            shuueki = result[1]
            if shuueki < 0.01: continue
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, shuuekiDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, shuuekiDict, zandakaDict = UpdateData(symbolListJP)
