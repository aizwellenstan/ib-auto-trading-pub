...
from modules.rironkabuka import GetRironFinancial, GetShuuekirisk
...
def HandleGetShuuekirisk(symbol):
    shuuekirisk = GetShuuekirisk(symbol)
    return [symbol, shuuekirisk]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, shuuekiriskDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetShuuekirisk, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            shuuekirisk = result[1]
            if shuuekirisk < -5: continue
            shuuekiriskDict[symbol] = shuuekirisk
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, shuuekiriskDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, shuuekiriskDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(shuuekiriskDict, open(shuuekiriskPath, "wb"))
