...
for symbol, industry in industryDict.items():
    ...
    #     # if "*" in str(ichikabuhaitou): 
    #     #     ichikabuhaitou = float(ichikabuhaitou.replace("*",""))
    #     # if "*" not in str(ichikabuhaitou): 
    #     if ichikabuhaitou != "-" and "*" not in str(ichikabuhaitou):
    #         if ichikabuhaitou < 5.0: continue
        # haitouseikouIdx = 2
        # haitouseikou = dividend[haitouseikouIdx]
        # if haitouseikou != "-":
        #     if haitouseikou == "赤字": continue
        # junnshisannhaitouritsuIdx = 4
        # junnshisanhaitouritsu = dividend[junnshisannhaitouritsuIdx]
        # if junnshisanhaitouritsu == "-": continue
    ...
    if dividendLength in [5, 6, 7, 10]:
        ...
        if len(plbs[3]) > 1:
            ...
            if (
                ...
                if jishakabukai/jishakabukai2 < 0.000001: continue
