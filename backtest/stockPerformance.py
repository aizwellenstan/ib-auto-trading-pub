...
def dump_result_list_to_csv(result_list, filename):
    ...
    with open(filename, mode='w', newline='') as csv_file:
        ...
        writer.writerow(['Symbol', 'Sharpe', 'Sortino'])
        for symbol, sharpe, sortino in result_list:
            writer.writerow([symbol, sharpe, sortino])
    print("DUMP FIN")
...
def HandleGetDataShort(symbol):
    res = GetDataShort(symbol)
    if len(res) < 1: return []
    sharpe = res[0]
    sortino = res[1]
    return [symbol, sharpe, sortino]

csvPath = f"{rootPath}/data/SharpeSortino.csv"
result_list_US = []
with ThreadPoolExecutor(max_workers=100) as executor:
    futures = [executor.submit(HandleGetDataShort, symbol) for symbol in symbolList]
    for future in as_completed(futures):
        result = future.result()
        ...
        sharpe = result[1]
        sortino = result[2]
        print(symbol, sharpe, sortino)
        result_list_US.append((symbol, sharpe, sortino))
        result_list_US.sort(key=lambda x: (x[1], x[2]), reverse=True)
        dump_result_list_to_csv(result_list_US, csvPath)