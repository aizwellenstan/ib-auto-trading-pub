...
from modules.financials import GetTreasuryShares
...
def HandleGetFreeCashflow(symbol):
    treasuryShares = GetTreasuryShares(symbol)
    return [symbol, treasuryShares]
...
def UpdateData(symbolList):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetFreeCashflow, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            treasuryShares = result[1]
            if len(treasuryShares) > 0:
                treasurySharesDict[symbol] = treasuryShares
    return treasurySharesDict

def CheckCheap(symbol, closeDictUS, rironkabukaDict, zandakaDict):
    close = closeDictUS[symbol]
...
def CheckCash(symbol, cashDictUS):
    if symbol not in cashDictUS: return [symbol, 0]
    data = cashDictUS[symbol]
...
def HandleDividendToPrice(symbol):
    dividendToPrice = GetDividendToPrice(symbol, 'USY')
...
def main(update=False):
    ...
    picklePathPerUS = f"{rootPath}/backtest/pickle/pro/compressed/dataDictPerUS.p"
    rironkabukaDict, zandakaDict = {}, {}
    if update:
        treasurySharesDict = UpdateData(symbolListUS)
        ...
        pickle.dump(treasurySharesDict, open(treasurySharesPath, "wb"))
        
        # pickle.dump(zandakaDict, open(zandakaPath, "wb"))
        # perDataDict = ThreadPool(symbolListUS, GetPer)
        # pickle.dump(perDataDict, open(picklePathPerUS, "wb"))
        print("pickle dump finished")
    else:
        rironkabukaDict = LoadPickle(rironkabukaPath)
        zandakaDict = LoadPickle(zandakaPath)
        perDataDict = LoadPickle(picklePathPerUS)

    cheapList = []
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(CheckCheap, symbol, closeDictUS, rironkabukaDict, zandakaDict) for symbol in symbolListUS]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
    ...
    # cashList = []
    # with ThreadPoolExecutor(max_workers=100) as executor:
    #     futures = [executor.submit(CheckCash, symbol, cashDictUS) for symbol in cheapList]
    #     for future in as_completed(futures):
    #         result = future.result()
    #         if len(result) < 1: continue
    #         symbol = result[0]
