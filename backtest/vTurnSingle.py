...
from modules.strategy.vTurn import Sandwitch
...
FUCK_ASIA_START = time_to_minutes("17:00")
...
def is_ny_op(time_str):
    ...
    return (
        ...
        # time_in_minutes >= JP_START and time_in_minutes < JP_END
    )
...
for symbol in symbolList:
    ...
    for i in range(0, len(oriNpArr)):
        npArr = oriNpArr[:i+2]
        signal, op, sl, tp = Sandwitch(npArr, tf, tick_val)
    ...
    class MyStrat(Strategy):
        ...
        def adjust_stop_loss(self, trade, new_stop_loss):
            ...
            if trade.is_long:
                if (
                    self.data.Close[-1] > trade.entry_price + (trade.entry_price - trade.sl) * 2
                ):
                ...
                if (
                    self.data.Close[-1] < trade.entry_price - (trade.sl - trade.entry_price) * 2
                ):
                    if new_stop_loss < trade.sl:
                        trade.sl = new_stop_loss
                #  if (
