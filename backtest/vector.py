...
if update:
    ...
    for symbol, close in closeJPDict.items():
        ...
        rironkabakaDict[symbol] = rironkabaka
...
# @njit
def calc_pl_for_bars(npArr, RR, lows, closes):
    position = 0
    balance = 1
    for i in range(4, len(npArr)-1):
        if (
            position == 0 and
            # npArr[i][0] > npArr[i-1][3]
            npArr[i-1][1] > npArr[i-1][2] and
            npArr[i-1][2] < npArr[i-1][3] and
...
def find_best_pl(npArr):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(calc_pl_for_bars, npArr, RR, lows, closes) for RR in RR_range]
...
csvPath = f"{rootPath}/data/vectorAttrJP.csv"
dump_result_list_to_csv(result_list_JP, csvPath)
csvPath = f"{rootPath}/data/vectorAttrUS.csv"
