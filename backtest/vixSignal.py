...
from modules.aiztradingview import GetClose, GetTradable
import pickle
...
# @njit
def Backtest(vixArr, vixCloseArr, npArr):
    print(vixArr[-1])
    npArr = npArr[-1058:]
    val = 0.01
    maxBalance = 1
    maxVal = 0
    for vixCloseLimit in vixCloseArr:
        balance = 1
        minRetrace = 1
        retraceList = np.empty(0)
        for i in range(1, len(npArr)):
            if vixArr[i-1][2] > vixCloseLimit:
                op = npArr[i-1][1]
                if op < 0.01: return 0
                tp = npArr[i][2]
...
#     symbol = 'VXX'
    ...
    balance = Backtest(vixArr, closeArr, npArr)
