...
def scanner(hourLimit, symbol, hour, minute):
    ...
    if (
        ...
        if (
            ...
            message = f"{symbol} (NoOption) 4barPlay reverse add buy op {op} sl {sl} tp {tp}"
            Alert(message)
        elif (
            npArr[-bar3*2+1][3] > npArr[-bar3*3][0] and
            high2 < high3 and
            ...
            message = f"{symbol} (NoOption) 4barPlay reverse add sell op {op} sl {sl} tp {tp}"
            Alert(message)

    if (
        (hour == hourLimit+1 and minute < 25) or
        hour < hourLimit+1
    ):
        bar3 = 13
        highSigArr1 = np.empty(0)
        ...
        if (
            ...
            sl = lowArrAll.min()
            tp = op + (op-sl) * 1.022222222
            message = f"{symbol} (NoOption) 4barPlay add buy op {op} sl {sl} tp {tp}"
            Alert(message)
        elif (
            npArr[-bar3*2+1][3] < npArr[-bar3*3][0] and
            high2 < high3 and
            ...
            sl = highArrAll.max()
            tp = op - (sl-op) * 1.022222222
            message = f"{symbol} (NoOption) 4barPlay add sell op {op} sl {sl} tp {tp}"
            Alert(message)

    if (
        (hour == hourLimit+3 and minute < 14) or
        hour < hourLimit+3
    ):
        bar8 = 25
        highSigArr1 = np.empty(0)
    ...
    if (
        (hour == hourLimit+4 and minute < 16) or
        hour < hourLimit+4
    ):
        bar9 = 26
        highSigArr1 = np.empty(0)
    ...
    if (
        (hour == hourLimit+3 and minute < 2) or
        hour < hourLimit+3
    ):
        bar8 = 30
        highSigArr1 = np.empty(0)
    ...
    if (
        (hour == hourLimit+2 and minute < 33) or
        hour < hourLimit+2
    ):
        bar9 = 21
        highSigArr1 = np.empty(0)
    ...
    if (
        (hour == hourLimit+5 and minute < 10) or
        hour < hourLimit+5
    ):
        bar10 = 29
        highSigArr1 = np.empty(0)
    ...
    if (
        (hour == hourLimit+6 and minute < 43) or
        hour < hourLimit+6
    ):
        bar11 = 18
        highSigArr1 = np.empty(0)
...
def shutDown():
    ...
    Alert(message)
    print(message)
    message = "WTF ARE U DOING, CLOSE YOUR FUCKING GOMI POJI"
    Alert(message)
    print(message)
    ...
    while(ib.sleep(1)):
        ...
        if sec == 0:
            ...
            for symbol in symbolList:
                scanner(hourLimit, symbol, hour, minute)
            print('tick')
        if dayLightSaving:
