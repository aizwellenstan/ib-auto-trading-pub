...
from modules.ib_option_chain import GetSPXBullPutCreaditSpread, GetSPXBullCallCreaditSpread, GetSPXCustomBullPutCreaditSpread, GetSPXBearPutCreaditSpread, GetSPXCustomBearPutCreaditSpread
...
def BacktestSpread(npArr, currency='USD', shortable=False,  basicPoint=0.01):
    ...
    combination = GetSPXCustomBearPutCreaditSpread(5)
    # print(combination)
    ...
    for comb in combination:
        ...
        for i in range(
            ...
            if npArr[i+daysLeft][3] > npArr[i][0] + spreadRange:
                if npArr[i+daysLeft][3] > comb['LongBE']:
                    capital += (npArr[i+daysLeft][3]-comb['LongBE']) * 100
                else:
                    capital -= comb['loss']
    ...
    message = f"CustomBearPutCreditSpread {bestCreaditSpread} \n"
