...
def HandleGetZandaka(symbol, dateList):
    for da in dateList:
        zandaka = GetZandaka(symbol, da)
        SaveZandaka(symbol, zandaka)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetZandaka, symbol, download) for symbol in symbolList]
