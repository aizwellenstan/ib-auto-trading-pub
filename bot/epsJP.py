...
from modules.financials import GetInventoryNetIncome
...
def HandleGetRironFinancial(symbol):
    rironkabuka, shisannkachi, shuueki, financialScore, financialDetail, shijou, gyoushu, haitourimawari, kiborisk = GetRironFinancial(symbol)
    return [symbol, rironkabuka, shisannkachi, shuueki, financialScore, financialDetail, shijou, gyoushu, haitourimawari, kiborisk]

def HandleGetZandaka(symbol):
    zandaka = GetZandaka(symbol)
    return [symbol, zandaka]

def HandleGetInventoryNetIncome(symbol):
    inventory, netIncome = GetInventoryNetIncome(symbol)
    return [symbol, inventory, netIncome]

def UpdateData(symbolList, gyoushuPath):
    gyoushuDict = LoadPickle(gyoushuPath)
    rironkabukaDict, shisannkachiDict, shuuekiDict, financialScoreDict, financialDetailDict, shijouDict, haitourimawariDict, kiboriskDict, zandakaDict = {}, {}, {}, {}, {}, {}, {}, {}, {}
    with ThreadPoolExecutor(max_workers=100) as executor:
        ...
        for future in as_completed(futures):
            ...
            shisannkachi = result[2]
            shuueki = result[3]
            financialScore = result[4]
            financialDetail = result[5]
            shijou = result[6]
            gyoushu = result[7]
            haitourimawari = result[8]
            kiborisk = result[9]
            if rironkabuka < 6: continue
            if shuueki < 0.001: continue
            if financialScore < 1: continue
            if len(financialDetail) < 5: continue
            if shijou == "": continue
            if gyoushu == "": continue
            if haitourimawari < 0: continue
            if kiborisk < -5: continue
            rironkabukaDict[symbol] = rironkabuka
            shisannkachiDict[symbol] = shisannkachi
            shuuekiDict[symbol] = shuueki
            financialScoreDict[symbol] = financialScore
            financialDetailDict[symbol] = financialDetail
            shijouDict[symbol] = shijou
            gyoushuDict[symbol] = gyoushu
            haitourimawariDict[symbol] = haitourimawari
            kiboriskDict[symbol] = kiborisk
    inventoryDict, netIncomeDict = {}, {}
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetInventoryNetIncome, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            ...
            netIncome = result[2]
            if len(inventory) > 0:
                inventoryDict[symbol] = inventory
            if len(netIncome) > 0:
                netIncomeDict[symbol] = netIncome
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, shisannkachiDict, shuuekiDict, financialScoreDict, financialDetailDict, shijouDict, gyoushuDict, haitourimawariDict, kiboriskDict, inventoryDict, netIncomeDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, shisannkachiDict, shuuekiDict, financialScoreDict, financialDetailDict, shijouDict, gyoushuDict, haitourimawariDict, kiboriskDict, inventoryDict, netIncomeDict, zandakaDict = UpdateData(symbolListJP, gyoushuPath)
        pickle.dump(rironkabukaDict, open(rironkabukaPath, "wb"))
        pickle.dump(shisannkachiDict, open(shisannkachiPath, "wb"))
        pickle.dump(shuuekiDict, open(shuuekiPath, "wb"))
        pickle.dump(financialScoreDict, open(financialScorePath, "wb"))
        pickle.dump(financialDetailDict, open(financialDetailPath, "wb"))
        pickle.dump(shijouDict, open(shijouPath, "wb"))
        pickle.dump(gyoushuDict, open(gyoushuPath, "wb"))
        pickle.dump(haitourimawariDict, open(haitourimawariPath, "wb"))
        pickle.dump(kiboriskDict, open(kiboriskPath, "wb"))
        pickle.dump(inventoryDict, open(inventoryPath, "wb"))
        pickle.dump(netIncomeDict, open(netIncomePath, "wb"))
