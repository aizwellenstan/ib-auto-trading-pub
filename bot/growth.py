...
from modules.irbank import GetZandaka, GetDividend, GetGrowth
...
def HandleGetGrowth(symbol):
    growth = GetGrowth(symbol)
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, growthDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetGrowth, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, growthDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, growthDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(growthDict, open(growthPath, "wb"))
