...
from modules.optionChain import GetAllCall
...
def BacktestSpread(symbol, npArr, currency='USD', shortable=False,  basicPoint=0.01):
    ...
    diaPrice, combination = GetAllCall(symbol)
    print(combination)
    ...
    for comb in combination:
        ...
        spreadRange = comb['LongBE'] - diaPrice
        capital = 0
        for i in range(
            1, len(npArr)-daysLeft-1
        ):
            longBE = npArr[i][0] + spreadRange
            if not npArr[i][6] == weekday: continue
            if npArr[i+daysLeft][3] > longBE:
                capital += (npArr[i+daysLeft][3]-longBE) * 100
...
'CDNS', 'JD', 'MS', 'RIVN', 'BIDU', 'FXC', 'DD', 'IRNT', 'GMBL', 'LIDR']
    ...
    profitSymList = ['AMC', 'AAPL', 'AMZN', 'AMD', 'BBBY', 'F', 'NVDA', 'WBD', 'DKNG', 'PLTR', 'BABA', 'GOOGL', 'CVNA', 'GME', 'SQ', 'MSFT', 'COIN', 'INTC', 'DWAC', 'RBLX', 'TAL', 'FDX', 'YETI', 'ITB', 'CVX', 'OXY', 'TGT',
'ARKK', 'NFLX', 'DLTR', 'W', 'UPS', 'MCK', 'XLF', 'ABC', 'DBC', 'FCG', 'GD', 'BMY', 'OKTA', 'GLD', 'HSY', 'FTNT', 'JNPR', 'COST', 'OIH', 'ORLY', 'REGN', 'PANW', 'AVGO', 'MU', 'PAYC', 'SPOT', 'ZS', 'TDOC', 'FOXA', 'AMAT', 'FL', 'FCX', 'CSIQ', 'HPE', 'DLR', 'TLRY', 'TWTR', 'EWG',
'XOM', 'EWU', 'TLT', 'V', 'C',
'FXY', 'NEE', 'UAA', 'CHWY', 'LCID', 'HOOD', 'T', 'ADBE', 'SI', 'NUGT', 'GM', 'KHC', 'ALB', 'MDT', 'X', 'TRV', 'KR', 'CDNS', 'JD', 'MS', 'RIVN', 'BIDU', 'FXC', 'DD', 'IRNT', 'GMBL', 'LIDR']
