...
def main():
    MIN_VOL = 1
    import pandas as pd
    df = pd.read_csv("cfd_vTurn_us.csv")
    ...
    contractMNQ, contractMES, contractN225M, contractN225MC, contractMNTPX, contractDJIA, contractMGC, contractQQQ = GetFuturesContracts(ib)
    tf = '5 mins'
    lastOpMap = {}
    while(ib.sleep(2)):
        ...
        for symbol, contract in contractDict.items():
            ...
            signal, op, sl, tp = VTurn(npArr, tf, tick_val=1)
        ...
        for symbol, res in tradeDict.items():
            ...
            closePos = False
            if signal != 0:
                msg = f'IRB {cfdContractDict[symbol].symbol} {tf} {signal} {op} {sl} {tp}'
