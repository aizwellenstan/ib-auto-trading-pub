...
from modules.trade.futures import GetCurrentPosition, CleanUp, GetTradeTime, GetFuturesContracts, ExecTrail, ExecTrailMOC, cancelUntriggeredAll
...
def onPendingTickers(tickers):
    global entropy_signal, ask, bid
    for t in tickers:
        # , t.high, t.low, t.close
        print(t.contract.symbol, t.bidSize, t.bid, t.ask, t.askSize)
        entropy_signal = CheckSignal(t.bidSize, t.askSize, t.close, ma_val, en_val, en_ma_val)
        ask = t.ask
        bid = t.bid

def main():
    global ma_val, en_val, en_ma_val
    MIN_VOL = 1
    ...
    ib.reqMktData(contractES, '', False, False)
    ib.pendingTickersEvent += onPendingTickers

    LAST_POSITION = GetCurrentPosition(ibc, contract)
    ...
    while(ib.sleep(3)):
        ...
        if currentTime.minute != lastMinute:
            ma_val, en_val, en_ma_val = generate_signal(dataDict["1 min"][:-1][:,3][-109:])
            lastMinute = currentTime.minute

        hour = currentTime.hour
        minute = currentTime.minute
        if hour == hourLimit - 1 and minute == 29:
            CleanUp(ibc, 1, contract, STOCKACCOUNT)
            CleanUp(ibc, -1, contract, STOCKACCOUNT)
        ...
        # if entropy_signal != 0:
        #     if LAST_POSITION != entropy_signal:
        #         CleanUp(ibc, entropy_signal, contract, STOCKACCOUNT)
        #         Alert(f"{contract.symbol} entropy {entropy_signal} CLEAN UP")
        #         LAST_POSITION = GetCurrentPosition(ibc, contract)
        ...
        for tf, res in tradeDict.items():
            ...
            if signal != 0:
                ...
                if tf not in lastOpMap or op != lastOpMap[tf]:
                    ...
                    oriOP = op
                    if vol > 1:
                        CleanUp(ibc, signal, contract, STOCKACCOUNT)
                        if signal != entropy_signal: 
                            print("signal != entropy", signal, entropy_signal)
                            continue
                        if signal > 0: op = ask
                        elif signal < 0: op = bid
                    ...
                    print(msg)
            if entropy_signal > 0: print("LONG")
            elif entropy_signal < 0: print("SHORT")
