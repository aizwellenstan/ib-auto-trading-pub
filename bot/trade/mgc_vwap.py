...
#         tradeDict = {}
#         for tf in tfs:
#             npArr = ibc.GetDataNpArr(contractMGC, tf)
#             if len(npArr) < 1: continue
#             signal, op, sl, tp = GetVwapStrategy(npArr, tick_val=0.1)
...
#             closePos = False
#             if signal != 0:
#                 msg = f'VWAP {contractMGC.symbol} {tf} {signal} {op} {sl} {tp}'
#                 vol = 1
#                 trades = ibc.GetOpenTrades()
#                 for trade in trades:
#                     if trade.contract == contractMGC:
...
#                     ExecTrade(ibc, signal, contractMGC, vol, op, sl, tp, closePos)
#                     Alert(msg)
#                     lastOpMap[tf] = op
