...
from modules.irbank import GetZandaka, GetDividend, GetValuePeg
...
def HandleGetValuePeg(symbol):
    valuepeg = GetValuePeg(symbol)
    return [symbol, valuepeg]
...
def UpdateData(symbolList):
    rironkabukaDict, financialScoreDict, financialDetailDict, valuepegDict, zandakaDict = {}, {}, {}, {}, {}
    ...
    with ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(HandleGetValuePeg, symbol) for symbol in symbolList]
        for future in as_completed(futures):
            result = future.result()
            if len(result) < 1: continue
            symbol = result[0]
            valuepeg = result[1]
            if len(valuepeg) < 2: continue
            valuepegDict[symbol] = valuepeg
    # with ThreadPoolExecutor(max_workers=100) as executor:
    ...
    return rironkabukaDict, financialScoreDict, financialDetailDict, valuepegDict, zandakaDict
...
def main(update=False):
    ...
    if update:
        rironkabukaDict, financialScoreDict, financialDetailDict, valuepegDict, zandakaDict = UpdateData(symbolListJP)
        ...
        pickle.dump(valuepegDict, open(valuepegPath, "wb"))
