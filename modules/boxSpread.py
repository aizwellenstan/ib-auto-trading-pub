...
def GetSPXCallOptionChainAll():
    ...
    newTickers = []
    for ticker in tickers:
        if ticker.bid < 0 or ticker.ask < 0: continue
        newTicker = {}
        newTicker['Expir'] = ticker.contract.lastTradeDateOrContractMonth
        newTicker['Strike'] = ticker.contract.strike
        newTicker['Bid'] = ticker.bid
...
def GetQQQPutOptionChain():
    ...
    strikes = [strike for strike in chain.strikes
            if strike % 1 == 0
            and qqqValue - 10 < strike < qqqValue + 10]
    expirations = sorted(exp for exp in chain.expirations)[:2]
    rights = ['P']
...
def GetSPYPutOptionChain():
    ...
    strikes = [strike for strike in chain.strikes
            if strike % 1 == 0
            and spyValue - 10 < strike < spyValue + 10]
    expirations = sorted(exp for exp in chain.expirations)[:2]
    rights = ['P']
...
def GetSPXBoxSpread():
    bullCallSpreadCombs = GetSPXBullCallCreaditSpread(5)
    bearPutSpreadCombs = GetSPXBearPutCreaditSpread(5)

    for bullCallComb in bullCallSpreadCombs:
        bullCallCombExpir = bullCallComb['Expir']
        for bearPutComb in bearPutSpreadCombs:
            bearPutCombExpir = bearPutComb['Expir']
            if bullCallCombExpir != bearPutCombExpir: continue
            bullCallProfit = bullCallComb['profit']
            bullCallLoss = bullCallComb['loss']
            bearPutProfit = bearPutComb['profit']
            bearPutLoss = bearPutComb['loss']
            if bullCallProfit - bullCallLoss + bearPutProfit - bearPutLoss > 0:
                if bullCallComb['SellStrike'] == bearPutComb['BuyStrike']:
                    print(f"bullCallComb {bullCallComb}")
                    print(f"bearPutComb {bearPutComb}")
