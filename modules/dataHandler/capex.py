...
FOLDER = f"{rootPath}/data/jp/capex"
...
def SaveCapex(symbol, capex):
    if len(capex) < 1:
        return 0
    
    try:
        # Convert capex to a pandas DataFrame
        df = pd.DataFrame(capex)
        df.columns = ['da', 'capex']
        
        # Convert 'capex' column to Int64
        df['capex'] = df['capex'].astype(pd.Int64Dtype())
        
    except Exception as e:
        # Error handling and logging
        print(f"{symbol} Capex Error: {e}")
        log_error(symbol, e)
        return -1
...
def GetCapex(symbol, col, da):
    try:
        ...
        # print(symbol, "Capex Error on line {}".format(sys.exc_info()[-1].tb_lineno))
        # print(e)
...
if __name__ == '__main__':
    # from modules.irbank import GetCapex
    symbol = '6810'
    # capex = GetCapex(symbol)
    # print(capex[-1])
    # res = SaveCapex(symbol, capex)
    # print(res)
    capex = GetCapex(symbol, 'capex', '2024-06-24')
    print(capex)