...
def SaveCf(symbol, npArr):
    if len(npArr) < 1: return 0
    try:
        filePath = f"{FOLDER}/{symbol}.parquet"
        df = pd.DataFrame(npArr)
        df.columns = ['da', 
            'eigyoucf', 'toushicf', 'saimucf',
...
cacheData = {}
def GetCf(symbol, col, da):
    global cacheData
    return get_col_before_da(cacheData, FOLDER, symbol, col, da)

ignoreSet = set()
def GetPlbscfcfArr(symbol, col, da):
    try:
        ...
        # print(symbol, "Plbscfcf Error on line {}".format(sys.exc_info()[-1].tb_lineno))
        # print(e)
        return []
...
if __name__ == '__main__':
    # from modules.irbank import GetPlbscfcf
    symbol = "6315"
    cf = GetCf(symbol, 'toushicf', "2022-03-01")
    print(cf)