...
FOLDER = f"{rootPath}/data/jp/holder"
...
cacheData = {}
ignoreSet = set()
def GetHolderArr(symbol, col, da):
    try:
        ...
        # print(symbol, "Holder Error on line {}".format(sys.exc_info()[-1].tb_lineno))
        # print(e)
        return []
...
cacheData = {}
def GetHolder(symbol, da=datetime.now()):
    try:
        global cacheData
        item = get_col_arr_before_da(cacheData, FOLDER, symbol, 'percentage', da)
        return item[-1]
    except Exception as e:
...
if __name__ == '__main__':
    # from modules.irbank import GetHolder
    symbol = "7003"
    # holder = GetHolder(symbol)
    # SaveHolder(symbol, holder)
    holder = GetHolder(symbol, '2024-07-21')