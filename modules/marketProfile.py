...
def find_levels( 
    ...
    # Get kernel of price. 
    kernal = stats.gaussian_kde(price, bw_method=atr*atr_mult, weights=weights)
    ...
    peaks, props = signal.find_peaks(pdf, prominence=prom_min)
    levels = [] 
    for peak in peaks:
...
def GetSupportResistance(npArr):
    data = pd.DataFrame(npArr, columns = ['open','high','low', 'close', 'volume', 'date'])
