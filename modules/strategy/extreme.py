...
def GetOPSLTP(npArr, signal, tf, tick_val, tp):
    # TP_VAL = 1.333333333
    if signal > 0:
        op = npArr[-1][0]
        sl = npArr[-2][2]
...
def Extreme(npArr, tf, tick_val=0.25):
    ...
    if np.max(npArr[:-1][-2:][:,1]) - np.min(npArr[:-1][-2:][:,2]) > atr * 3:
        ...
        TP_VAL = 1.2
        period = 2
        ll = np.min(npArr[:-2][-period:][:,2])
        # tp = np.max(npArr[:-2][-period:][:,2])
        tp = ll - (npArr[-2][3]-ll)
        # if (
        #     npArr[-2][2] < ll and
        #     npArr[-2][3] > ll
        ...
        #     # if pe[-1] < sma[-1]:
        #     signal = 1
        #     op, sl, tp = GetOPSLTP(npArr, signal, tf, tick_val, tp)
