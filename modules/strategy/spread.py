...
def GetMovesToSweep(op, targets, reverse=False):
...
def Spread(npArr, npArr2, tf, tick_val=0.25):
    if len(npArr) < 192 or npArr[-2][4] < 1 or npArr[-2][5] < 1 or npArr[-3][5] < 1:
        return 0, npArr[-1][0], 0, 0
    op, maxRRLong, maxRangeLong, maxRRShort, maxRangeShort = GetSweep(npArr, 30, tick_val)
    # rr = 2.5
    # if IS_NQ: rr = 9
    ratio = npArr[:-1][-40:][:,3] / npArr2[:-1][-40:][:,3]
    ratioMa = SmaArr(ratio, 40)
    rr = 3
    if (
        ratio[-1] < ratioMa[-1] and
        ratio[-1] < ratio[-3] and
        ratio[-1] < ratio[-2] and
        npArr[-2][5] > npArr[-3][5] * 1 and
        npArr[-2][4] / npArr[-2][5] > npArr[-3][4] / npArr[-3][5]
        # npArr[-2][1] > npArr[-3][1] and
        # maxRRLong > maxRRShort
    ):
        signal = 1
        tp = op + maxRangeLong
        sl = npArr[-2][2]
