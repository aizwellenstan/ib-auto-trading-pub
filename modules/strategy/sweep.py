...
def GetOPSLTP(npArr, signal, tp, tf, tick_val):
    # TP_VAL = 1.333333333
    # TP_VAL = 2
    if signal > 0:
        op = npArr[-1][0]
...
def GetMovesToSweep(atr, op, targets, reverse=False, tick_val=0.25):
    targets.sort(reverse=reverse)
    maxRR = 0
    maxRange = 0
    c = 0
    for t in targets:
        if abs(t - op) <= tick_val * 11: continue
        c += 1
        if abs(t - op) > atr * 1.4: continue
        rr = c / abs(t - op)
        if rr > maxRR:
            maxRR = rr
            maxRange = abs(t - op)
...
def GetSweep(npArr, period=30, tick_val=0.25):
    ...
    for i in range(0, period):
        high = npArr[-1-period+i][1]
        low = npArr[-1-period+i][2]
        high1 = npArr[-1-period+i-1][1]
        low1 = npArr[-1-period+i-1][2]
        unSweepHigh[high] = 1
        unSweepLow[low] = 1
        unSweepHighLow[low] = 1
        unSweepLowHigh[high] = 1
        ...
        for periousHigh in list(unSweepHighLow.keys()):
            if high1 > periousHigh:
                unSweepHighLow.pop(periousHigh)
        for periousLow in list(unSweepLowHigh.keys()):
            if low1 < periousLow:
                unSweepLowHigh.pop(periousLow)      
    op = npArr[-2][3]
    atrPeriod = 8
    atr = ATR(npArr[:-1][:,1][-atrPeriod-1:],npArr[:-1][:,2][-atrPeriod-1:],npArr[:-1][:,3][-atrPeriod-1:], atrPeriod)
    maxRRLong, maxRangeLong = GetMovesToSweep(atr, op, list(unSweepHigh.keys())+list(unSweepHighLow.keys()), False, tick_val)
    maxRRShort, maxRangeShort = GetMovesToSweep(atr, op, list(unSweepLow.keys())+list(unSweepLowHigh.keys()), True, tick_val)
    return op, maxRRLong, maxRangeLong, maxRRShort, maxRangeShort

def Sweep(npArr, tf, tick_val=0.25, IS_NQ=False):
    if len(npArr) < 192 or npArr[-2][4] < 1:
        return 0, npArr[-1][0], 0, 0
    atrPeriod = 200
    if tf not in ['5 mins']: atrPeriod = 100
    atr = ATR(npArr[:-1][:,1][-atrPeriod:],npArr[:-1][:,2][-atrPeriod:],npArr[:-1][:,3][-atrPeriod:])
    if np.max(npArr[:-1][-2:][:,1]) - np.min(npArr[:-1][-2:][:,2]) > atr * 2.5:
        op, maxRRLong, maxRangeLong, maxRRShort, maxRangeShort = GetSweep(npArr, 30, tick_val)
        # print("RRL", maxRRLong, maxRangeLong)
        # print("RRS", maxRRShort, maxRangeShort)
        rr = 2.5
        # if IS_NQ: rr = 9
        if (
            npArr[-2][1] > npArr[-3][1] and
            maxRRLong > maxRRShort and
            ...
            tp = op + maxRangeLong
            sl = npArr[-2][2]
            # if IS_NQ: sl = floor_round(op-(op-sl)/4, tick_val)
            if op - sl > tick_val:
                slippage = tick_val * 2
            ...
            tp = op - maxRangeShort
            sl = npArr[-2][1]
            # if IS_NQ: sl = ceil_round(op+(sl-op)/4, tick_val)
            if sl - op > tick_val:
                slippage = tick_val * 2
