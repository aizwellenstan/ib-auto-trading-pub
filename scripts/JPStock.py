...
def bracketStopLimitOrderTwoTargets(c,
    ...
    print("limitPrice "+str(limitPrice))
    print("stopPrice "+str(stopPrice))
    print("stopLossPrice "+str(stopLossPrice))
    print("takeProfitPrice1 "+str(takeProfitPrice1))

    print("limit price ", limitPrice, " stop price ",stopPrice)
...
def CheckForOpen():
    ...
    scanCodeList = ["LOW_OPT_VOL_PUT_CALL_RATIO",
    ...
    for scanCode in scanCodeList:
        # Scanner
        hot_stk_by_volume = ScannerSubscription(instrument='STOCK.HK',
                                                locationCode='STK.HK.TSE_JPN', # 'STK.US.MAJOR',
                                                scanCode=scanCode,
                                                # belowPrice='20',
                                                # abovePrice='0.01',
                                                aboveVolume='409'  # <1407
                                                )

        scanner = ib.reqScannerData(hot_stk_by_volume, [])

        #scanner = pd.read_csv (r'Screener Results.csv')
