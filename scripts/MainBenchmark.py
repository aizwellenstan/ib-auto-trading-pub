...
risk = 0.00613800895 * 0.18549364406 * 0.63 #0.79 #0.378 #0.06
...
def checkHisBarsD1(contract, shift, hisBarsD1, hisBarsD1reverse, symbol, maxHigh):
    ...
    if hisBarsM1[0].close > maxHigh: return True
    ...
    HCHL = (
                (hisBarsD1[1-shift].high - hisBarsD1[1-shift].close)
                / (hisBarsD1[1-shift].high - hisBarsD1[1-shift].low)
    )
    HCHL2 = (
    ...
    if (
        hisBarsD1[1-shift].close/hisBarsD1[momentumPeriod].close > avgMomentum
    ): return False
...
def checkScanner(shift,symbol):
    try:
        ...
        maxHigh = hisBarsD1reverse[1-shift].high
        for i in range(2-shift, 98):
            if hisBarsD1reverse[i].high > maxHigh:
                maxHigh = hisBarsD1reverse[i].high
                
        if not checkHisBarsD1(contract,shift,hisBarsD1,hisBarsD1reverse,symbol,maxHigh):
            return False
            ...
            stock_list.append(
                {
                    's':symbol,
                    'close1':hisBarsD1reverse[1-i].close,
                    'maxHigh': maxHigh
                }
            )
        return True
...
def check_for_open():
    ...
    for stock in stock_list:
        try:
            ...
            bid = ticker.bid
            retryCount = 0
            while (math.isnan(bid) or bid < 0) and retryCount < 4:
                print("retry")
                ticker=ib.reqMktData(contract, '', False, False)
                ib.sleep(3)
                ask = ticker.ask
            ...
            if ask > stock['close1'] * 1.115132275: continue
...
def main():
    ...
    while(ib.sleep(1)):
        ...
        if(hour == 13 and min == 17 and sec == 0):
            update_balance()
            get_scanner()
            remove_duplicate()
            print(scanner)
