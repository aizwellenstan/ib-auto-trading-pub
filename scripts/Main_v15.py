...
duplicate_list = ['SQQQ', 'PLTR', 'SPCE', 'CSCO']
all_symbol_list = []
gain_sym_list = []
vol_sym_list = []
scanner = []
...
# Scanner
def get_scanner():
    global scanner, gain_sym_list, vol_sym_list
    gain_sym_list = []
    vol_sym_list = []
    ...
    scanner = gain_sym_list + vol_sym_list
    remove_duplicate()
...
def check_gap(symbol):
    try:
        ...
        if(len(hisBarsD1) < 6): return
        print("check_gap",symbol,hisBarsD1[0].date)

        op = hisBarsD1[0].open
        if not check_op_limit(op): return
        ...
        if hisBarsD1[0].open > hisBarsD1[1].close:
            ...
            if buy > 0:
                ...
                w1_morning_star_val, w1_evening_star_val, w1_engulfin_val = talib_pattern(
                    10080, today, df_w1)
                if(w1_engulfin_val < 0):    return

                d1_morning_star_val, d1_evening_star_val, d1_engulfin_val = talib_pattern(
                    1440, today, df_d1)
                if(d1_evening_star_val < 0):    return
                if(d1_engulfin_val < 0):    return
                
                talib_passed_list_d1_w1.append(symbol)
                gap_list.append(symbol)
                print(symbol)
...
def check_for_pre_open():
    for symbol in scanner:
        try:
            ...
            if(
                hisBarsD1[0].open <= hisBarsH8[1].high
            ):  continue
            ...
            if symbol not in talib_passed_list_d1_w1:
                d1_morning_star_val, d1_evening_star_val, d1_engulfin_val = talib_pattern(
                    1440, today, df_d1)
                if(d1_evening_star_val < 0):
                    continue
                if(d1_engulfin_val < 0):
                    continue

                w1_morning_star_val, w1_evening_star_val, w1_engulfin_val = talib_pattern(
                    10080, today, df_w1)
                if(w1_engulfin_val < 0):
                    continue

            if symbol not in fillterPassedList:
            ...
            # biasQQQH3 = (hisBarsQQ1H3[1].close 
            ...
            log_m15_turn_arround = 0
            log_three_bar_play_W1 = 0
            logMorningStar = 0
            logTurnArround = 0
            logBuySetup = 0
            ...
            # M15 Turn arround
            if(
                hisBarsM15[2].close < hisBarsM15[2].open
                and hisBarsM15[1].close > hisBarsM15[1].open
            ):
                buy += 1
                log_m15_turn_arround += 1
            ...
            if((buy > 0 or sell > 0) and buy != sell):
                if(buy > sell):
                    ...
                    if(log_bias > 0):
                        stock_list_bias.append(symbol)
                    if(log_bp3_d1 > 0):
                        stock_list_bp3_d1.append(symbol)
                    ...
                    # Log
                    if(log_m15_turn_arround): log(symbol + " m15_turn_arround")
                    if(log_three_bar_play_W1>0): log(symbol + " three_bar_play_W1")
                    ...
                    if(log_bp3_d1>0): log(symbol + " Bp3")
                    if(logBp4>0): log(symbol + " Bp4")
                    if(log_bias>0): log(symbol + " Bias")
...
def check_m30_turn_arround():
    today = ib.reqCurrentTime().today()
    for symbol in fillterPassedList:
        if symbol in stock_list: continue
        try:
            contract = Stock(symbol, 'SMART', 'USD')
            ...
            hisBarsM30 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='2 D',
                barSizeSetting='30 mins', whatToShow='ASK', useRTH=True)
            df_m30 = pd.DataFrame(hisBarsM30)
            hisBarsM30 = hisBarsM30[::-1]
            ...
            if((buy > 0 or sell > 0) and buy != sell):
                if(buy > sell):
                    m30_morning_star_val, m30_evening_star_val, m30_engulfin_val = talib_pattern(
                        30, today, df_m30)
                    if(m30_evening_star_val < 0):   continue
                    stock_list.append(symbol)
                    talib_passed_list.append(symbol)
                    if(logM30TurnArround>0):
                        stock_list_M30_turn_arround.append(symbol)
        ...
        try:
            ...
            if(ask>0 and bid>0):
                ...
                if(op != sl):
                    ...
                    if symbol in stock_list_bias:
                        tpBaseVal = 20.5
                    if symbol in stock_list_M30_turn_arround:
                        tpBaseVal = 22.5
                    if symbol in stock_list_bp3_d1:
                        tpBaseVal = 25.9
                    for marketcap in stock_list_marcap:
                    ...
                    if(vol>=3):
                        ...
                        if (spread < (op - sl) * 0.27):
                            ...
                            if(abs((op-sl)/sl)<diff or abs(op-sl)<=0.01):
                                ...
                                if symbol not in talib_passed_list:
                                    hisBarsM30 = ib.reqHistoricalData(
                                        contract, endDateTime='', durationStr='2 D',
                                        barSizeSetting='30 mins', whatToShow='ASK', useRTH=True)
                                    df_m30 = pd.DataFrame(hisBarsM30)
