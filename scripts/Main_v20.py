...
risk = 0.00613800895 * 0.33 #0.79 #0.378 #0.06
...
duplicate_list = ['SOXL','PLTR','SPCE','CSCO','SHIP','ABNB','FIVE','ALF','OPINL',
'SDHY','PSTH','AEI']
all_symbol_list = []
gain_sym_list = []
vol_sym_list = []
...
def checkHisBarsD1(contract, shift, hisBarsD1, hisBarsD1reverse, symbol):
    ...
    if symbol in jared and op > 100: return True
    ...
    if not (
        hisBarsM1[0].close > hisBarsD1[1-shift].high
    ): return False

    # buy = 0
    # highBreakPeriodArr = []
    # for i in range(1, len(hisBarsD1)):
    #     if hisBarsD1[i].close > hisBarsD1[i].open:
    #         count = 0
    #         for j in range(i+2, len(hisBarsD1)):
    #             if (
    #                 hisBarsD1[i].open > hisBarsD1[j-1].high
    #                 and hisBarsD1[i].open < hisBarsD1[j].high
    #             ):
    #                 count = j
    #             else: break
    #         highBreakPeriodArr.append(count)

    # avgHighBreakPeriod = int(sum(highBreakPeriodArr)/len(highBreakPeriodArr))

    # if avgHighBreakPeriod > 2:
    #     maxHigh = hisBarsD1[1-shift].high
    #     for i in range(2-shift, avgHighBreakPeriod+1):
    #         if hisBarsD1[i].high > maxHigh:
    #             maxHigh = hisBarsD1[i].high
        
    #     if hisBarsM1[0].close > maxHigh: buy += 1
    ...
    # if buy < 1:
        ...
        # if (
        #     df.iloc[-2+shift]['bias'] > avgBearBias
        # ): return False
    ...
    if op < 100:
        if (
            hisBarsD1[1].close < hisBarsD1[240].close * 2.64583333
        ): return False
...
def checkScanner(shift,symbol):
    try:
        ...
        if len(hisBarsD1) < 330: return False
...
def check_for_open():
    ...
    for stock in stock_list:
        try:
            ...
            bid = ticker.bid
            retryCount = 0
            while (math.isnan(bid) or bid < 0) and retryCount < 7:
                print("retry")
                ticker=ib.reqMktData(contract, '', False, False)
                ib.sleep(3)
                ask = ticker.ask
            ...
            if op > stock['close1'] * 1.115132275: continue

            print("symbol ",symbol," bid " +str(bid)," ask ",str(ask))

            if(ask>0 and bid>0):
                if not check_op_limit(op): continue
                sl = normalizeFloat(op - 0.14, ask, bid)
                if op >= 16.5:
                    sl = normalizeFloat(op * 0.9930862018, ask, bid)
                if op > 100:
                    sl = normalizeFloat(op * 0.9977520318, ask, bid)
                if(sl != 0 and op != sl):
                    tpVal = 2
                    tp = op + (op-sl) * tpVal
                    ...
                    # if vol * op > cash * 0.5:
                    #     vol = int(cash*risk*0.33/(op-sl))
                    if(vol>=volLimit):
                        spread = 0
                        spread = ask-bid
                        if (spread < (op - sl) * 0.32 and spread < 0.3):
                            log("BuyStop " + symbol
                                    + " vol " + str(vol)
                                    + " op " + str(op)
                                    + " sl " + str(sl)
