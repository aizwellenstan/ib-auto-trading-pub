...
from modules.aiztradingview import GetPerformance,GetProfit,GetPerformanceMore,GetProfitWithADR,GetDR,GetREIT,GetIndustry,GetRvol
...
def checkHisBarsD1(contract, shift, hisBarsD1, symbol):
    ...
    if not (
        hisBarsD1[1-shift].open > hisBarsD1[2-shift].close * 1.04
    ):
...
def check_for_open():
    ...
    for stock in stock_list:
        try:
            ...
            hisBarsD1 = ib.reqHistoricalData(
                contract, endDateTime='', durationStr='101 D',
                barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            ...
            if not (
                ...
                if gapRange/stock['adr'] > 0.85: continue
            

            adr = (
                abs(hisBarsD1[-2].close - hisBarsD1[-3].open) +
                abs(hisBarsD1[-4].close - hisBarsD1[-5].open) +
                abs(hisBarsD1[-6].close - hisBarsD1[-7].open) +
                abs(hisBarsD1[-8].close - hisBarsD1[-9].open) +
                abs(hisBarsD1[-10].close - hisBarsD1[-11].open)
            ) / 5
            ...
            adr = (
                abs(hisBarsD1[-2].close - hisBarsD1[-5].open) +
                abs(hisBarsD1[-6].close - hisBarsD1[-9].open) +
                abs(hisBarsD1[-10].close - hisBarsD1[-13].open) +
                abs(hisBarsD1[-14].close - hisBarsD1[-17].open) +
                abs(hisBarsD1[-18].close - hisBarsD1[-21].open)
            ) / 5
            ...
            if not (
                ...
                adr = (
                    abs(hisBarsD1[-2].close - hisBarsD1[-6].open) +
                    abs(hisBarsD1[-7].close - hisBarsD1[-11].open) +
                    abs(hisBarsD1[-12].close - hisBarsD1[-16].open) +
                    abs(hisBarsD1[-17].close - hisBarsD1[-21].open) +
                    abs(hisBarsD1[-22].close - hisBarsD1[-26].open)
                ) / 5
            ...
            if not (
                ...
                adr = (
                    abs(hisBarsD1[-2].close - hisBarsD1[-8].open) +
                    abs(hisBarsD1[-9].close - hisBarsD1[-15].open) +
                    abs(hisBarsD1[-16].close - hisBarsD1[-22].open) +
                    abs(hisBarsD1[-23].close - hisBarsD1[-29].open) +
                    abs(hisBarsD1[-30].close - hisBarsD1[-36].open)
                ) / 5
            ...
            adr = (
                abs(hisBarsD1[-2].close - hisBarsD1[-18].open) +
                abs(hisBarsD1[-19].close - hisBarsD1[-35].open) +
                abs(hisBarsD1[-36].close - hisBarsD1[-52].open) +
                abs(hisBarsD1[-53].close - hisBarsD1[-69].open) +
                abs(hisBarsD1[-70].close - hisBarsD1[-86].open)
            ) / 5
            ...
            if(ask>0 and bid>0):
                ...
                if stock['adr'] > 0.14:
                    sl = normalizeFloat(op - stock['adr'] * 0.35, 0.01)
                slByClose = 0
                if (
                    hisBarsD1[-3].close > hisBarsD1[-3].open and
                    hisBarsD1[-2].close > hisBarsD1[-2].open
...
def main():
    ...
    while(ib.sleep(1)):
        ...
        # EOD Limit
        if(hour == 15 and min == 41 and sec == 0):
            close_all_limit()
            
        # EOD
        if(hour == 15 and min == 42 and sec == 0):
            close_all()
