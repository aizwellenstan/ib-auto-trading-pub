...
contractIWM = Stock('IWM', 'SMART', 'USD')
contractUVXY = Stock('UVXY', 'SMART', 'USD')
contractTLT = Stock('TLT', 'SMART', 'USD')
contractXLU = Stock('XLU', 'SMART', 'USD')
contractXLI = Stock('XLI', 'SMART', 'USD')
...
def getMarketCondition():
    global hisBarsQQQD1, hisBarsSPYD1, hisBarsVTID1, hisBarsDIAD1, hisBarsIWMD1, hisBarsUVXYD1, hisBarsTLTD1, hisBarsXLUD1, hisBarsXLID1
    ...
    hisBarsUVXYD1 = ib.reqHistoricalData(
    contractUVXY, endDateTime=endDateTimeD1, durationStr='365 D',
    barSizeSetting='1 day', whatToShow='ASK', useRTH=True)

    hisBarsTLTD1 = ib.reqHistoricalData(
    contractTLT, endDateTime=endDateTimeD1, durationStr='365 D',
    barSizeSetting='1 day', whatToShow='ASK', useRTH=True)

    hisBarsXLUD1 = ib.reqHistoricalData(
    contractXLU, endDateTime=endDateTimeD1, durationStr='365 D',
    barSizeSetting='1 day', whatToShow='ASK', useRTH=True)

    hisBarsXLID1 = ib.reqHistoricalData(
    contractXLI, endDateTime=endDateTimeD1, durationStr='365 D',
    barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
...
def checkHisBarsD1(shift, hisBarsD1, symbol):
    ...
    try:
        ...
        if symbol in shortableSymList:
            ...
            if upper < -0.23: return False

            upper, lower = PlotLines(hisBarsDIAD1)
            if lower < -0.61: return False
            if upper < -0.34: return False

            upper, lower = PlotLines(hisBarsIWMD1)
            if lower < -0.42: return False

            upper, lower = PlotLines(hisBarsUVXYD1)
            if lower > 0.11: return False

            upper, lower = PlotLines(hisBarsTLTD1)
            if upper < -0.12: return False

            upper, lower = PlotLines(hisBarsXLUD1)
            if lower < -0.26: return False

            upper, lower = PlotLines(hisBarsXLID1)
            if lower < -0.29: return False
        get_linenumber()
        ...
        if predictPrice < hisBarsD1[1-shift].close * 0.32: return False
        get_linenumber()
        ...
        if symbol in shortableSymList:
            ...
            if lower < -0.17: return False
            if upper < -0.45: return False
        ...
        if predictPrice < hisBarsD1[1-shift].close * 0.58: return False
        get_linenumber()
        ...
        if symbol not in hisBarsD1Dict:
            ...
            if symbol in shortableSymList:
                hisBarsD1 = ib.reqHistoricalData(
                    contract, endDateTime=endDateTimeD1, durationStr='365 D',
                    barSizeSetting='1 day', whatToShow='ASK', useRTH=True)
            else:
                hisBarsD1 = ib.reqHistoricalData(
