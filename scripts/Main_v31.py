...
def checkHisBarsD1(vwapDf, symbol):
    try:
        ...
        if len(vwapDf) > 200:
            ...
            SmaD1 = np.mean(closeArr[-200:])
            if (
                closeArr[-1] < SmaD1 and
                closeArr[-1] > vwapDf.iloc[-1].Vwap
            ): return False
        get_linenumber()
        ...
        if len(vwapDf) > 3:
            if (
                vwapDf.iloc[-1].High < vwapDf.iloc[-2].Low and
                vwapDf.iloc[-1].High > vwapDf.iloc[-3].Low and
                vwapDf.iloc[-1].High < vwapDf.iloc[-4].Low
            ): return False
        get_linenumber()
...
def checkScanner(shift, symbol):
    try:
        ...
        if not checkHisBarsD1(vwapDf,symbol):
            ...
            try:
                (z, x1, x2, x3, x4, x5) = GetZScore(symbol)
                # if z < -12.684417255779115: return False
                # get_linenumber()
                # if x1 < -0.08726128279417748: return False
                # get_linenumber()
                # if x2 < -11.465461481051497: return False
                # get_linenumber()
                # if x3 < -0.16533203914907385: return False
                # get_linenumber()
                if x4 < 0.05672075995801045: return False
                get_linenumber()
                if x5 < -0.07445506268457697: return False
                get_linenumber()
            except:
                print("Error on line {}".format(sys.exc_info()[-1].tb_lineno))
                return False
