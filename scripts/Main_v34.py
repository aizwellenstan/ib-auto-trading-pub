...
def getMarketCondition():
    ...
    diaDf = stockInfo.history(period="365d")
    mask = diaDf.index < str(timeD.date())
    diaDf = diaDf.loc[mask]
    df = diaDf[['Open','High','Low','Close']]
    npArr = df.to_numpy()
...
def checkHisBarsD1(vwapDf, symbol):
    try:
        ...
        if len(vwapDf) > 1:
            if vwapDf.iloc[-1].Volume < 62691: return False
        get_linenumber()
        ...
        if len(vwapDf) > 1:
            if vwapDf.iloc[-1].Close/vwapDf.iloc[-1].Vwap>1.6239156980011462:return False
        get_linenumber()
...
def checkOpen():
    ...
    for stock in stockList:
        try:
            ...
            if hisBarsD1[-1].open/hisBarsD1[-2].close < 0.9717444717444716: 
                continue
            if IsTesting: print(symbol)
            ...
            if(ask>0 and bid>0):
                op = 0.0
                if currency == 'USD':
                    op = max(preMaxHigh,ask) + 0.01 * 19
                else:
                    op = ask + 0.01 * 19
                if op < hisBarsD1[-2].close * 1.0206422018348624: continue
                if op > total_cash/39.75406504065041: continue
                ...
                if(vol >= volLimit):
                    tp = stock['tp']
                    if (tp-op) / (op-sl) < 1:
                        tp = op + (op-sl) * 113.14285714285668
                        tp = NormalizeFloat(tp, basicPoint)
                    spread = 0
                    spread = ask-bid
                    spreadPercent = 0.32
...
def checkScannerFilter(currency):
    ...
    if (
        ...
        "CARV" not in performanceSymList
    ):
