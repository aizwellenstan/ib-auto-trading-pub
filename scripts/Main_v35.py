...
from modules.aiztradingview import GetPerformance,GetADR,GetDR,GetREIT,GetPreVolume,GetRvol,GetPreVolumeDict,GetPerformanceJP,GetDailyWinner,GetDailyWinnerJP
...
def getMarketCondition():
    global isMarketDown, isBearish

    vtidf = GetDf('VTI')
    diadf = GetDf('DIA')

    if (len(diadf) > 200 and len(vtidf) > 200):
...
def GetSL(op, adr):
    ...
    if adr > 0.47:
        if op - sl < basicPoint * 231:  
            sl = op - basicPoint * 231
    else:
        if (
...
def checkHisBarsD1(vwapDf, symbol, adr, isMarketDown):
    try:
        ...
        if len(vwapDf) > 1:
            if vwapDf.iloc[-1].Volume < 20100: return False
        get_linenumber()

        if len(vwapDf) > 1:
            if vwapDf.iloc[-1].High-vwapDf.iloc[-1].Low < 0.06: return False
        get_linenumber()
        ...
        if isMarketDown:
            if len(vwapDf) > 2:
                if vwapDf.iloc[-2].High-vwapDf.iloc[-2].Low > 0:
                    if (
                        (
        ...
        if len(vwapDf) > 4:
            if (
                vwapDf.iloc[-4].Close > vwapDf.iloc[-4].Open and
                vwapDf.iloc[-3].Close < vwapDf.iloc[-3].Open and
                vwapDf.iloc[-2].Close < vwapDf.iloc[-24].Open and
                vwapDf.iloc[-1].Close < vwapDf.iloc[-1].Open and
...
def checkScanner(shift, symbol):
    try:
        ...
        if not checkHisBarsD1(vwapDf, symbol, adr, isMarketDown): return False
        ...
        try:
            ...
            if(ask>0 and bid>0):
                ...
                if currency == 'USD':
                    ...
                    op = ask + basicPoint * 19
                # if op < hisBarsD1[-2].close * 1.020086083213773: continue
                if op < hisBarsD1[-2].close * 0.9479836353009935: continue
                op = NormalizeFloat(op, basicPoint)
                if stock['adr'] > 0.63: sl = op - stock['adr'] * 0.4
                elif stock['adr'] > 0.14: sl = op - stock['adr'] * 0.35
                else: sl = op - stock['adr'] * 0.05
                ...
                if stock['adr'] > 0.47:
                    ...
                    if op - sl < basicPoint * 74:
                        sl = op - basicPoint * 74
                if stock['adr'] > 0.01:
                    if (stock['adr']/(op-sl)) < 1.295: continue
                ...
                if (tp-op) / (op-sl) < 2.308823529411767:
                    ...
                    tp = NormalizeFloat(tp, basicPoint)
                tp = op + (op-sl) * 1.2837837837837844
                tp = NormalizeFloat(tp, basicPoint)
                vol = stock['vol']
                spread = 0
                spread = ask-bid
    ...
    while(ib.sleep(1)):
        ...
        if currency == 'USD':
            ...
            if dayLightSaving:
                ...
                if(hour == 13 and minute == 40 and sec == 0):
                    checkOpen()
