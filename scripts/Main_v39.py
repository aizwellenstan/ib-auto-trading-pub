    ...
    try:
        ...
        if not checkHisBarsD1(vwapDf, symbol, adr): return False
            ...
            if "institutionsFloatPercentHeld" in shareholders:
                ...
                if floatPercentHeld > 10.5128: return False
                get_linenumber()
            ...
            if(ask>0 and bid>0):
                ...
                if (spread < (op - sl) * spreadPercent and spread < spreadFixed):
                    ...
                    if(abs((op-sl)/sl)<diff or abs(op-sl)<=0.01):
                        ...
                        if stock['haveOptionChain']:
                            optionDict[symbol] = op
            else:
...
def CheckOptionOpen(optionDict, chainsDict):
    newOptionDict = dict(optionDict)
    tradedList = []
    for symbol, op in optionDict.items():
        if symbol in tradedList: break
        high = GetData(symbol)[-1][1]
        if high > op:
            chains = chainsDict[symbol]
...
def CheckOption():
    ...
    for symbol, op in optionDict.items():
        ...
        chainsDict[symbol] = chains

    if len(optionDict) > 0:
        while(ib.sleep(1)):
            optionDict = CheckOptionOpen(optionDict, chainsDict)
            if len(optionDict) < 1: break
...
def UpdateKeepOpenList(keepOpenList, csvPath, count):
    portfolioList = []
    if os.path.exists(csvPath):
        df = pd.read_csv(csvPath)
        if count < 1: count = len(df)
        portfolioList = list(df.Symbol.values)[:count]
    for symbol in portfolioList:
        if symbol in keepOpenList: continue
        keepOpenList.append(symbol)

sharpePortfolioPath = './data/Sharpe.csv'
portfolioPath = './data/PortfolioRiron.csv'
exDividendPath = './data/ExDividend.csv'
reversePath = './data/Reverse.csv'
shinyouzanPath = './data/ShinyouzanUS.csv'
squeezeTradePath = '/data/Squeeze.csv'
rootPath = "."
...
# UpdateKeepOpenList(keepOpenList, portfolioUSEtfPath, 11)
UpdateKeepOpenList(keepOpenList, portfolioUSStockPath, 21)
# UpdateKeepOpenList(keepOpenList, portfolioUSLevergeEtfPath, 10)

print(keepOpenList)

if os.path.exists(sharpePortfolioPath):
    df = pd.read_csv(sharpePortfolioPath)
...
exDividendList = []
if os.path.exists(exDividendPath):
    df = pd.read_csv(exDividendPath)
    exDividendList = list(df.Symbol.values)

for symbol in exDividendList:
    keepOpenList.append(symbol)

# reverseList = []
# if os.path.exists(reversePath):
#     df = pd.read_csv(reversePath)
...
shinyouzanList = []
if os.path.exists(shinyouzanPath):
    df = pd.read_csv(shinyouzanPath)
    shinyouzanList = list(df.USSymbol.values)
...
# squeezeList = []
# if os.path.exists(squeezeTradePath):
#     squeezeTradeDict = LoadDict(squeezeTradePath,'sl')
#     squeezeList = squeezeTradeDict.keys()

# for symbol in squeezeList:
#     keepOpenList.append(symbol)
print('keepOpenList',keepOpenList)
