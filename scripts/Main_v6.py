...
hot_stk_by_bull = ScannerSubscription(instrument='STK',
                                        locationCode='STK.US.MAJOR',
                                        scanCode='BULLISH_PPO_DIST',
                                        belowPrice=cash*risk,
                                        abovePrice='11.57',
                                        aboveVolume='149455'  # <1407
                                        )
...
bullList = ib.reqScannerData(hot_stk_by_bull, [])

gainSymList = []
bullSymList = []
duplicateList = [duplicateSym]

for stock in gainList:
    symbol = stock.contractDetails.contract.symbol
    gainSymList.append(symbol)

for stock in bullList:
    symbol = stock.contractDetails.contract.symbol
    bullSymList.append(symbol)

scanner = list(set(gainSymList).intersection(bullSymList))
print("scanner before filter ", scanner)
...
while(ib.sleep(1)):
    ...
    if(hour == 13 and min == 45 and sec==0):
        ...
        print(stockList)
    # Pre Market Scanner
    if(hour == 14 and min == 28 and sec==0):
        cashDf = pd.DataFrame(ib.accountValues())
        cashDf = cashDf.loc[cashDf['tag'] == 'CashBalance']
        cashDf = cashDf.loc[cashDf['currency'] == 'USD']
        cash = float(cashDf['value'])
