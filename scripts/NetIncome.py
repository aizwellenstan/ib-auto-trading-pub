...
portfolioPath = './data/NetIncome.csv'
import os
...
for symbol in keepOpenList:
    ibSymbol = symbol
    try:
        if "." in symbol: ibSymbol = symbol.replace("."," ")
    except: continue
    if symbol in closeDictJP:
        portfolioDict[ibSymbol] = closeDictJP[symbol]/usdjpy
    if symbol in closeDict:
        portfolioDict[ibSymbol] = closeDict[symbol]
...
positions = ibc.GetPositions()
# print(positions)
for symbol, vol in tradeDict.items():
    if symbol in closeDict:
