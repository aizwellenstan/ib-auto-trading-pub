import pandas as pd
from modules.aiztradingview import GetCloseAll
...
portfolioPath = './data/ShinyouzanUS.csv'
import os
if os.path.exists(portfolioPath):
    df = pd.read_csv(portfolioPath)
    keepOpenList = list(df.USSymbol.values)
...
divPortfolioPath = './data/shinyouzanPortfolio.csv'
df = pd.DataFrame()
